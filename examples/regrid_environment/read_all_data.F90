...
END FUNCTION inquire_frac
  ...
  CALL hdlerr( NF90_INQ_VARID(il_file_id, TRIM(cl_nam),  il_indice_id),  w_unit, subname, __FILE__, __LINE__ )
  !
  ila_what(:)=1
  !
  ila_dim(1)=nlon
...
END SUBROUTINE read_mask
  ...
  !
  cl_nam=cl_grd//".srf" 
  CALL hdlerr( NF90_INQ_VARID(il_file_id, cl_nam, il_srf_id),  w_unit, subname, __FILE__, __LINE__ )
  !
  ila_what(:)=1
  !
  ila_dim(1)=nlon
...
END SUBROUTINE read_area
  ...
  !****************************************************************************************
  SUBROUTINE read_frac (nlon,nlat, &
                        data_filename, cl_grd, w_unit, file_debug, &
                        gridfrc)
  !****************************************************************************************
  !
  ...
  INTEGER,  DIMENSION(2)   :: ila_what
  !
  REAL (kind=wp), DIMENSION(nlon,nlat)  :: gridfrc
  !
  !
  ! Dimensions
  !
  ...
  ! Data
  !
  CALL hdlerr( NF90_GET_VAR (il_file_id, il_srf_id, gridfrc, &
     ila_what, ila_dim), __LINE__ )
  IF (file_debug) THEN
      WRITE(w_unit,*) '... global grid frac reading done'
      CALL FLUSH(w_unit)
  ENDIF
  !
