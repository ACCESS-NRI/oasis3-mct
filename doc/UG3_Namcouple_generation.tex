...
\item all lines beginning with \# are ignored and considered as
  comments;
\item blank lines are supported (since OASIS3-MCT\_4.0).
\end{itemize}
...
Compared to OASIS3, several {\it namcouple} inputs have been deprecated
in OASIS3-MCT (i.e. they will not be read and not be used)
but, for backwards compatibility, they can still appear in the {\it namcouple}.  These
inputs will be noted ``UNUSED'' in the following text and will not be fully described.
...
are described in chapter \ref{sec_transformations}. A realistic {\it
  namcouple} can be found in {\tt
  oasis3-mct/examples/} sub-directories.
...
ocean and an atmosphere components are
coupled. The ocean running on grid {\tt toce} provides only the SOSSTSST field to the atmosphere (grid {\tt atmo}),
which in return provides the field CONSFTOT to the ocean. The field
CONSFTOT is also explicitly written out to a file {\tt flda3.nc} and one field SOALBEDO is read from a file.
...
\item {\tt \$NLOGPRT}: The first, second and third numbers on the line below
  ...
  and (iii) the  load balancing analysis. The information is written by OASIS3-MCT
  ...
  \begin{itemize}
  \item 0 : production mode. One file {\tt debug.root.xx} is open by the master process of
    each component and one file {\tt debug\_notroot.xx} is open for all the
    other processes of each component to write only error information.
  \item 1 : one file {\tt debug.root.xx} is open by the master process of
    each component to write information equivalent to level 10 (see
    below) and memory usage information;
    one file {\tt debug\_notroot.xx} is open for all the other
    processes of each component to write error information.
  \item 2 : one file {\tt debug.yy.xxxxxx} is open by each process of each
    component (with {\tt yy} being the component number and {\tt xxxxxx} the process number)
    to write normal production diagnostics and memory usage information
  \item 5 : as for 2, and some initial debug info
  \item 10: as for 5, and the routine calling tree
  \item 12: as for 10, and some routine calling notes
  \item 15: as for 12, and even more debug diagnostics
  \item 20: as for 15, and some extra runtime analysis
  \item 30: full debug information
  \end{itemize}
  The second number defines if time statistics are written out to
  file {\tt comp\_name.timers\_xxxx} (with {\tt comp\_name} being the component name, see section \ref{init_comp}); it can be:
  \begin{itemize}
...
The third number can be set to 1 to activate a {\bf load balancing} diagnostic.
 ...
 To facilitates this work, OASIS3-MCT can output the full timeline of all coupling related events, for any of the allocated resources. This timeline is saved in one netCDF file per coupled component ({\tt timeline\_xxx\_component.nc}). It provides the comprehensive sequence of any operations related to the coupling (field exchange through MPI, field output on disk, field interpolation and mapping, field reading on disk, restart writing, initialisation and termination phase of the OASIS3-MCT setup) so that any simulation slow down in link with the use of the coupler can be identified.
...
\item {\tt \$NCDFTYP}: Optional; on the line below
  this keyword is a character string that indicates the NetCDF file type
  for all (i.e. mapping, restart, output) new NetCDF files generated by OASIS3-MCT\footnote{NetCDF will read and/or overwrite existing files of any NetCDF file type, and the file type will remain unchanged in that case.}.
  Options are {\tt cdf1},
  {\tt cdf2}, and {\tt cdf5}. The mode {\tt cdf1}, which is the default, is also known as classic mode,  {\tt cdf2} as large file format or {\tt 64bit\_offset} and supports larger
  files, {\tt cdf5} as {\tt 64bit\_data} and supports both larger files
  and larger variables.  More information about these file types can
  be found in the NetCDF documentation.   
  The file format for any NetCDF file can be diagnosed by using {\tt ncdump -k filename'}.
...
\item {\tt \$NMAPDEC}: Optional; on the line below this keyword is a character string
  that indicates the mapping decomposition value to be used during local mapping of the source/target field on the target/source component processes during the remapping.  The
  options are {\tt decomp\_1d} and {\tt decomp\_wghtfile}.  Option {\tt decomp\_1d} decomposes the field in a simple
  one dimensional way while {\tt decomp\_wghtfile} decomposes the field using the
  ...
  The default is {\tt decomp\_1d} but it is recommended to test {\tt decomp\_wghtfile} to see if it improves performance. 
  More details can be found in \cite {craig18} and in \cite{valcke11}.
...
\item {\tt \$NNOREST}: Optional; on the line below this keyword is a character
  ...
  a restart (for instance, variables with LAG $>$ 0) if the restart file does not exist. By default, or if {\tt \$NNOREST} is not true, missing
...
entry on the field first line ({\tt EXPORTED}, {\tt IGNOUT} or {\tt
  INPUT} in the example above). The field may be\footnote{The status {\tt AUXILARY} is not supported anymore.} :

\begin{itemize}
\item {\tt EXPORTED} (or {\tt IGNORED}): exchanged  by OASIS3-MCT
\item {\tt EXPOUT} (or {\tt IGNOUT}): exchanged and also written to two
...
\item {\tt OUTPUT}: written out to an output debug NetCDF file by the
  ...
  times corresponding to the period indicated by the user in
  the {\it namcouple}.
...
The first 4 lines for fields with status {\tt EXPORTED} and {\tt
...
\item Field first line:
  ...
  \item 86400 : coupling or I/O period for the field, in seconds or any other units as long as they are the same in all components and in the {\it namcouple}
  \item 3 : number of transformations to be performed by OASIS3-MCT on
    this field, described on the fourth line
  ...
  \item {\tt EXPOUT} : field status, see above
  \end{itemize}
\item Field second line:
  ...
  \item 64 : number of points for the target grid second dimension
    ...
    These source and target grid dimensions are optional but have to be provided
    in order to have 2D fields written as 2D arrays in the debug files;
...
this field. Choices are {\tt LOCTRANS}, {\tt CHECKIN},  {\tt BLASOLD}, {\tt  MAPPING}, {\tt YAC}, {\tt SCRIPR}, {\tt CONSERV}, {\tt BLASNEW}, {\tt CHECKOUT}.
In addition, there is one or more configuring lines describing some parameters for each transformation. These transformations and configuring
lines are described in chapter \ref{sec_transformations}.

\end{itemize}

{\bf Support to group multiple fields in a single communication}

With OASIS3-MCT, it is possible to group multiple fields in a
...
All fields in this group will then use the same
{\it namcouple} settings (source and target grids, transformations, etc.) for that entry.
In the component model codes,
these fields are still apparently sent or received one at a
time through individual {\tt oasis\_put} and {\tt oasis\_get}. Inside OASIS3-MCT, the fields in this group are stored and a single mapping
...
transformations to reduce communication costs by aggregating multiple
...
This option is compatible with the special case of 3D unbundled fields
and it is particularly well suited for lowering the latency of the
communications of such large objects.
...
\begin{itemize}
  \item Field first line:
    ...
    \item the source symbolic name must be repeated twice on the field
      first line
    \item 1 : UNUSED but still required for parsing
    \item 86400 : output period for the field, in seconds or any other units as long as they are the same in all components and in the {\it namcouple}
  ...
  \item Field second line:
    ...
    \item 64 : number of points for the target grid second dimension (optional)$^{4}$
...
\item 86400: input period in seconds or any other units as long as they are the same in all components and in the {\it namcouple}
