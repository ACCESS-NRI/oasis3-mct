...
\item {\tt \$NCDFTYP}: Optional (new in Oasis3-MCT\_5.0); on the line below
  ...
  for all (i.e. mapping, restart, output) NetCDF files generated by Oasis.  
  The options are {\tt cdf1},
  {\tt cdf2}, and {\tt cdf5}.  cdf1 is also known as classic mode.  cdf2 is 
  also known as large file format or 64bit_offset and supports larger
  files.  cdf5 is also known as 64bit_data and supports both larger files
  and larger variables.  Because cdf5 may not be generally available in
  all NetCDF installs, use of this option requires that the C preprocessor
  directive {\tt CDF_64BIT_DATA} be used when building Oasis.  If that
