...
\item Update examples, add SPOC

\item No interfacing with other regridding library but deep
  analysis and environment on how to use SCRIP or ESMF or XIOS to
  generate and reuse weights ticket 2349

\item New load balancing tool

\item locally conservative remapping / runoffs

\item python , C and C++ interface

\item Migration from SVN to GIT for source management, C3, IS3,
  ticket 2352

\item Migration of the OASIS3-MCT web site from DKRZ (content
  manager Plone) to Cerfacs (content manager Wordpress) ticket 2697

\item Update to MCT2.11

\item Update of compiling environment / Support of gnu 10 compilers for MCT (ticket 2689 + Makefile update to generate Oasis libraries, ticket 2509

\item Extension of the oasis\_get\_intracomm API routine to support
  multiple components, C2, ES2, ticket 2687 (closed)
  
  The API routine oasis\_get\_intracomm merges two MPI communicators from two different components to form a new communicator. It is proposed to extend this to more than 2 components by overloading the interface. This is needed in coupled models involving both OASIS3-MCT and XIOS when XIOS manages ensemble simulations.
    
\item Minor issues
  \begin{itemize}
  \item Adding the number of neighbours used in the remapping file
    name
  \item Easy detection of the target grid point that do not
    receive any value ticket 731
  \item Allow the definition of north\_thresh and south\_thresh values
    (Lambert projection) through the namcouple, IS3, ticket 2459
  \item Sending/receiving simple scalars ticket 1937
  \item Systematic tests of NetCDF returned error code, C2, IS3, ticket 550 
  \end{itemize}
    
\item The {\tt oasis\_def\_var} interface was overloaded to support excluding the argument, {\tt id\_var\_shape}, from the argument list.  This argument is no longer used.  For backwards compatibility, it can still be passed.

\item The {\tt BLASNEW} operation was extended to support combining fields together.

\item The {\tt CHECKIN} and {\tt CHECKOUT} log file output was updated
  to include more diagnostics as well as mask, area, and fraction
  weighted means and sums. ticket 2356 et 1761

\item  {\tt DESTARTR}, {\tt FRACARTR} or {\tt FRACNNTR} options : the ``True Area'' (TR) correction

  New options for {\tt SCRIPR CONSERV} to have an exact conservation of the field surface-integrated values, based on the "TRue" (TR) area of the cells. Details can be found in section \ref{subsec_interp} and in \cite{valcke19}. Note that to activate these TR options, the true area of the cell must be provided in the auxiliary file {\it areas.nc} in {\bf square radians}.

\item New {\tt GSSPOS} and {\tt BSSPOS} for global CONSERV / Option GLBPOS of CONSERV redistribution ticket 2327

  These new options operate as {\tt GLBPOS} and {\tt BASPOS} except that the multiplicative term is computed separately for positive
  and negative values of the field. More details can be found in section \ref{subsec_cooking} and in \cite{craig19}.

\item New {\tt \$NCDFTYP} netcdf file format setting

  This new option, {\tt \$NCDFTYP}, allows the user to set the netcdf file format type for all NetCDF output files written by OASIS3.  See {\tt \$NCDFTYP} in section \ref{subsec_namcouplefirst}.
...
\item Bugfixes
  ...
  \item Update oasis\_abort, write to unit 0, ticket 2678
  \item cplfind array bounds check, ticket 2655
  \item Fix error in maxloops/kfac computation in
    oasis\_mpi\_reducelists, ticket 2654
  \item Local distance in GAUSWGT interpolation ticket 2500
  \item LUCIA compatibility with CRAY compiler ticket 2473 
  \item Abort with NOINTERP when only a subdomain of the global grid is coupled, ticket 2472
  \item Problem with BILINEAR remapping from COSMO to NEMO, ticket 2419
  \item Problem in m\_MCTWorld.F90 in the routine "initm, ticket 1321
