...
\section{Changes between OASIS3-MCT\_6.0 and OASIS3-MCT\_5.0}

The last version of the coupler, OASIS3-MCT\_6.0, comes with the
following new functionalities and improvements :

\begin{itemize}

\item {\bf YAC interpolation library} :  interfacing with the parallel remapping algorithms for 2D interpolation on the sphere from version 3.10 of the YAC coupler  \citep{hanke2016} \citep{hanke2019}  all details are provided in section \ref{subsec_yac} and in \cite{jonville2024}.

\item {\bf Support of dynamic fractions}, see {\tt fracwgt} option in section \ref{prismput} and in appendix \ref{subsec_fracwgt}.

\item An {\bf upgraded compiling environment} based on {\bf two environment variables \$(OASIS\_COUPLE) and \$(OASIS\_ENV)}, see section \ref{subsec_compile}.

\item {\bf Support of a new ``Cube" partition}, see section \ref{Cube_partition}, adapted to specific
...
\item Suppression of big NetCDF files in the git repository and its history in order to drastically reduce its size, see Redmine issue \#2907

\item Migration of the OASIS3-MCT web site from DKRZ (content manager Plone) to Cerfacs (content manager Wordpress) in July 2022.

\item Migration of the CI/CD (Continuous Integration and Continuous Deployment) environment from Buildbot to gitlab .

\item Different bug fixes, see the following Redmine closed issues :
\begin{itemize}
\item  \#3098, an array bound mismatch in mod\_oasis\_load\_balancing.F90 ;
\item   \#2900, a bug with partition duplication in case the grid dimensions were not declared in the {\it namcouple} ;
\item   \#2841, the support of multiple grids for one same partition (linked with \#2900) ;
\item   \#2816, a freeze in the load balancing analysis when a component was not at all involved in the coupling ;
\item   \#2812, an uninitialized array in grid writing routines ;
\item   \#2793, wrong {\it namcouple} processing for BLASNEW ;
\item   \#2725, a problem with BICUBIC/BILINEAR sequence in weights generation ;
\item   \#2704, a problem with coupled = .false. in oasis\_init\_comp.
\end{itemize}
\end{itemize}

\section{Changes between OASIS3-MCT\_5.0 and OASIS3-MCT\_4.0}

The last version of the coupler, OASIS3-MCT\_5.0, comes with the
following novelties:

\begin{itemize}

\item Python, C and C++ bindings, see sections \ref{APIC},
  \ref{APIpython} and \ref{subsec_equivalent}

\item A new  load balancing analysis tool, see section \ref{lucia}

\item An environment to use either SCRIP, ESMF or XIOS to
  generate regridding weights and to analyse the quality of the
  regridding, see directory {\tt
    examples/regrid\_environment} and section \ref{subsec_regrid}

 \item A new locally conservative remapping, to be used in particular
   for runoffs, see {\tt LOCCUNIF}, {\tt LOCCDIST} and {\tt LOCCGAUS} in section \ref{subsec_interp}

 \item Extension of {\tt BLASNEW} operation to support combination of
   coupling fields, see section \ref{subsec_cooking}

\item Improved and additional diagnostics in {\tt CHECKIN} and {\tt
    CHECKOUT}, see sections \ref{subsec_preproc}
  and \ref{subsec_cooking}

\item {\tt SCRIPR/CONSERV} option for normalisation by the true area of the grid cells,
  see {\tt DESTARTR}, {\tt FRACARTR} or {\tt FRACNNTR} in section
  \ref{subsec_interp}

\item New {\tt GSSPOS} and {\tt BSSPOS} options for global CONSERV (as
  {\tt GLBPOS} and {\tt BASPOS} options respectively except that the
  multiplicative term is computed separately for positive
  and negative values of the field), see section \ref{subsec_cooking}

  \item Extension of {\tt oasis\_get\_intracomm} to support
  multiple components, see {\tt oasis\_get\_multi\_intracomm} in  section \ref{subsubsec_auxroutines}

\item Communication/exchange of simple scalars, see section \ref{subsec_scalar}

\item Update examples in {\tt oasis3-mct/examples} directory, see
  section \ref{subsec_running}

\item Update to MCT 2.11

\item Update of compiling environment, see \ref{subsec_compile}

\item Migration from SVN to GIT for source management

\item Migration of the OASIS3-MCT web site from DKRZ (content
  manager Plone) to Cerfacs (content manager Wordpress), see the new
  site at {\tt https://oasis.cerfacs.fr/en/}

\item For {\tt DISTWGT}, {\tt DISTWGTNF}, {\tt GAUSWGT},
  {\tt GAUSWGTNF}  {\tt LOCCUNIF}, {\tt LOCCDIST} or {\tt LOCCGAUS},
  inclusion of the number of neighbours used in the remapping file
  name

\item  For {\tt SCRIPR/CONSERV}, specification through the {\it namcouple} of North thresh
  {\tt \$NTHRESH} and South thresh {\tt \$STHRESH} values, above
  which a Lambert projection is activated

\item New {\tt \$NCDFTYP} NetCDF file format setting through the
    {\it namcouple}, see section \ref{subsec_namcouplefirst}

\item Overload {\tt oasis\_def\_var} interface to support excluding the argument {\tt id\_var\_shape} from the argument list

\item Systematic tests of NetCDF returned error code

\item Update oasis\_abort to also write to unit 0 (stderr)

\item Bugfixes

  \begin{enumerate}
  \item Component name argument in oasis\_get\_intercomm (ticket \#2776)
  \item BICUBIC/BILINEAR sequence in weights generation (ticket \# 2725)
  \item Array bounds check for cplfind (ticket \# 2655)
  \item Fix error in maxloops/kfac computation in
    oasis\_mpi\_reducelists (ticket \# 2654)
  \item Exact calculation of local distance in {\tt GAUSWGT}
    interpolation (ticket \#2500)
  \item Compatibility of load balancing analysis tool with CRAY
    compiler (ticket \# 2473)
  \item Problem with {\tt SCRIPR/BILINEAR} periodicity (ticket \# 2419)
  \item Problem in {\tt m\_MCTWorld.F90} in the routine {\tt initm} (ticket \# 1321)
  \item Argument {\tt id\_var\_nodim} defined as IN in {\tt
      mod\_oasis\_var.F90} to compile with NEMO\_4.0 and NEMO trunk (git commit 28f4fe59)
  \item In {\tt SCRIPR/CONSERV} , condition on coincidence
    of segments is reinitialised to false for each cell (bugfix ``lcoinc''  ; see \cite{valcke19})
  \end{enumerate}
...
before in OASIS3.3 (see section \ref{subsec_lag})}.
...
OASIS3-MCT retains prior interface names of OASIS3.3
  ...
  compatibility. However, new interface names such as {\tt
    oasis\_put} are also available and should be prefered. 
...
In OASIS3.3, components for which only a subset of processes
  ...
  oasis\_set\_couplcomm} are now available to create or set a coupling
  communicator gathering only these processes (see section \ref{subsec_MPI1}). (Note: this was further improved in OASIS3-MCT\_3.0.)
...
\item Support to group multiple fields in a single communication.
...
{\tt ATMTAUX:ATMTAUY:ATMHFLUX  TAUX:TAUY:HEATFLUX 1 3600 3 rstrt.nc EXPORTED}
 ...
 in a single {\it namcouple} entry. All fields in this group will use the
...
%  unique.
The inverse feature is not allowed: a single target (get) field
