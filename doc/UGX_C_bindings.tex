...
the return code is consistent with the {\tt
  kinfo} argument in the Fortran interfaces.  For this functions the
following constructs are equivalent in the the two languages:
...
For conveniency a similar macro {\tt OASIS\_CHECK\_MPI\_ERR} has been
defined for testing the return code of any MPI function against {\tt
  MPI\_SUCCESS} and cleanly aborting OASIS in case of failure.\\
...
\vspace{0.2cm}
More information about all these interfaces can be found in the Fortran interface section (see \ref{API}).  Notes and deviations from the Fortran standard are noted below.
...
\item {\tt int oasis\_c\_init\_comp(int* mynummod, const char* cdnam, const bool coupled)}
  \begin{itemize}
  \item This {\tt init\_comp} interface includes the {\tt coupled} flag but
...
\item {\tt int oasis\_c\_create\_couplcomm(const int icpl, const MPI\_Comm allcomm, MPI\_Comm* cplcomm)}
  \begin{itemize}
  \item {\tt allcomm} is a C {\tt MPI\_Comm} type input argument (by value)
  \item {\tt cplcomm} is a C {\tt MPI\_Comm*} type output argument (by reference)
  \end{itemize}
...
\item {\tt int oasis\_c\_def\_partition(int* id\_part, const int kparal\_size, const int* kparal, const int ig\_size, const char* name)}
  \begin{itemize}
  \item {\tt kparal\_size} is the size of the array {\tt kparal}: the
  ...
  \item the {\tt oasis\_c.h} header also provides a set of predefined
    ...
    kparal}, namely {\tt OASIS\_Strategy}, {\tt OASIS\_Segments}, {\tt
  ...
  \item the partition strategy, to be stored in the {\tt
    OASIS\_Strategy} position of {\tt kparal}, can take one of the
...
\item {\tt int oasis\_c\_write\_grid(const char* cgrid, const int nx, const int ny, const int nx\_loc, const int ny\_loc, const double* lon, const double* lat, const int partid)}
  \begin{itemize}
    \item {\tt nx} and {\tt ny} are the first and second dimensions
      (usually lon and lat) of the global grid
    \item {\tt nx\_loc} and {\tt ny\_loc} is the two-dimensional size
    ...
    \item {\tt partid} is relevant only for parallel writing. In case
...
\item {\tt int oasis\_c\_write\_corner(const char* cgrid, const int nx, const int ny, const int nc, const int nx\_loc, const int ny\_loc, const double* clo, const double* cla, const int partid)}
  \begin{itemize}
    ...
    \item {\tt nx\_loc} and {\tt ny\_loc} is the two-dimensional size
      of the local arrays {\tt lon} and {\tt lat}
    \item {\tt clo} and {\tt cla} are stored with a fortran compatible
      (column major) ordering. If they are declared as triple
      pointers, they have to be {\tt clo[nc][ny\_loc][nx\_loc]}
    \item {\tt partid} is relevant only for parallel writing. In case
      of single proc invocations, the constant {\tt OASIS\_No\_Part}
      can be passed as a placeholder
  \end{itemize}
...
\vspace{0.2cm}
\item {\tt int oasis\_c\_def\_var(int* id\_nports, const char* cdport, const int id\_part, const bundle\_size, const int kinout, const int ktype)}
  \begin{itemize}
  \item {\tt bundle\_size} is a scalar integer, corresponding to the
    second entry of the fortran {\tt var\_nodim} array
  ...
  \item {\tt ktype} can receive one of the two predefined constants
...
\vspace{0.2cm}
\item {\tt int oasis\_c\_put(const int var\_id, const int kstep, const
  ...
  \item {\tt bundle\_size} should be coherent with the argument of
    same name in {\tt oasis\_c\_def\_var}
...
\vspace{0.2cm}
\item {\tt int oasis\_c\_get(const int var\_id, const int kstep, const
  int x\_size, const int y\_size, const int bundle\_size3, const int
  fkind, const int storage, void* fld1, int* kinfo)}
  \begin{itemize}
  \item refer to {\tt oasis\_c\_put} for the arguments explanations.
  \end{itemize}
...
\vspace{0.2cm}
\item {\tt int oasis\_c\_put\_inquire(int varid, int msec, int* kinfo)}
  \begin{itemize}
  \item {\tt kinfo} contains a return status to be compared against
    the values of the {\tt return\_codes} enumeration in {\tt oasis\_c.h}
  \end{itemize}
