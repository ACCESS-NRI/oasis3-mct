...
string on the first configuring line for each field in the {\it
  namcouple} and the restart field in that file must have the sympbolic name of the coupling field in the {\it
  namcouple}, (see section \ref{subsec_namcouplesecond}). If the coupling exchange involves higher-order fields (see section \ref{subsubsec_sendingreceiving}), the higher-order restart fields in the coupling restart file must have the same name than the coupling field itself prefixed with either {\tt av2\_} , {\tt av3\_} , {\tt av4\_} , or {\tt av5\_}. For example, higher-order restart fields for the {\tt SOSSTSST} field in section \ref{subsubsec_secondEXPORTED}, if needed, would have to be named {\tt av2\_SOSSTSST}, {\tt av3\_SOSSTSST}, {\tt av4\_SOSSTSST}, and {\tt av5\_SOSSTSST}.
...
\ref{subsec_griddata} above).  The shape and indexing of each
