...
With OASIS3-MCT, the grid data files {\em grids.nc, masks.nc} and {\em
  areas.nc} are required for certain operations:
\begin{itemize}
  \item{\em grids.nc} and {\em masks.nc} for all {\tt SCRIPR} regriddings (see
    section \ref{subsec_interp})
  \item in addition, {\em areas.nc} for {\tt SCRIPR} conservative remapping
...
\item {\em masks.nc} and {\em areas.nc} for global {\tt
    CONSERV} (see section \ref{subsec_cooking}).
\end{itemize}
...
\item {\em masks.nc}: contains the masks for all component model grids
 ...
 File {\em masks.nc} may also contain the grid cell fractions that
 defines the active (i.e. not masked) part of the cells.  Fractions
 should be consistent with the mask field. The fraction fields, if present, are only
 ...
 missing.  Note that by OASIS3-MCT convention for the mask, a gridcell
