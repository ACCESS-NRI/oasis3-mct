...
\item  {\tt pyoasis         }  = builds and installs shared-cbindings package for use with python (NOTE: python is not fully supported)
...
To interface a component code with OASIS3-MCT and use {\tt mod\_oasis} (see section \ref{subsubsec_Use}), it is often adequate to just include and link the psmile objects, modules (in {\tt \$ARCHDIR/build/lib/psmile.MPI1}), and libraries (in {\tt \$ARCHDIR/lib/libpsmile.MPI1.a}) during the compilation and linking.
\footnote{If module {\tt mod\_prism} is used in the models, it is necessary to
include and link both the psmile and the mct objects, modules, and libraries
...
For double precision coupling fields, there is no need to promote REAL variables to DOUBLE PRECISION at compilation.
