...
in the {\tt oasis3-MCT/util/make\_dir} directory
with the makefile {\tt TopMakefileOasis3}.
In the new OASIS3-MCT\_6.0 compiling environment,
two environment variables {\tt \$(OASIS\_COUPLE)} and {\tt \$(OASIS\_ENV)} have to be defined:
\begin{itemize}
\item {\tt \$(OASIS\_COUPLE)} gives {\bf the location of the coupler sources} ({\tt /lib, /util, /examples}, etc. directories); 
\item {\tt \$(OASIS\_ENV)} gives {\bf the extension of the header Makefile to be used for compilation}.
\end{itemize}
{\tt TopMakefileOasis3} includes {\tt make.inc}
which includes the header Makefile {\tt make.\$(OASIS\_ENV)} specific to the hardware and compiling environment used. 
We recommend to compose {\tt \$(OASIS\_ENV)} as {\tt (computer\_name)\_(compiler\_version)\_(MPI\_library\_version)}. A file named \newline {\tt comp\_env\_\$(OASIS\_ENV).sh} should also exist the compiling directory. It defines the computer environment for running coupled model, in particular the test examples from {\tt oasis3-mct/examples}, and should to be activated in the user environment.

Several header files {\tt make.\$(OASIS\_ENV)} and {\tt comp\_env\_\$(OASIS\_ENV).sh} files are distributed with the release and can be used as
a template to create a custom file for your machine. In those header files, appropriate options must be activated for compiling OASIS3-MCT in debug mode or with OpenMP, when applicable (see respectively the environment variables {\tt \$(OASIS\_DEBUG)} and {\tt \$(OASIS\_OPENMP)} in those files).
Note tha is mandatory that the coupled model components be compiled with the same compilers and system software than the OASIS3-MCT library itself.

After successful compilation, resulting
libraries are found in the directory in {\tt \$(ARCHDIR)/lib}\footnote{where \$(ARCHDIR) = \$(OASIS\_COUPLE)/INSTALL\_OASIS.\$(OASIS\_ENV), see the header Makefiles} while
...
available and python codes are now fully supported. Therefore, the Makefile {\tt TopMakefileOasis3}
...
\item  {\tt help                =} provides information about the different targets               
\end{itemize}
...
The following CPP keys can be used when compiling OASIS3-MCT, in general to be specified in the {\tt CPPDEF} variable in the header Makefile :

\begin{itemize}

\item{\tt YAC\_REMAP}: to compile the YAC interpolation library, see section \ref{subsec_interp}. Note that this CPP key can be activated by specifying {\tt ENABLE\_YAC = YES} in the header Makefile.

\item {\tt LOCTRANS\_RESTARTS}: to allow partial time transformations, if any, to be saved at the end of the run in the coupling restart files for exact restart of the next run. {\bf Note that this is not backward compatible as {\tt LOCTRANS} restart fields were saved by default in OASIS3-MCT\_5.0.} Although we do not recommend it as it may have unexpected side effects, this allows a run to end in the middle of a coupling period. Note that this CPP key can be activated by specifying {\tt OASIS\_LOCTRANS\_RESTARTS = YES} in the header Makefile. See section \ref{subsec_timetrans} for important remarks regarding the use of this CPP key.
...
\item {\tt TREAT\_OVERLAY}:  to ensure, in {\tt SCRIPR/CONSERV}
  ...
  index will not be considered (they also can be both masked); mandatory
  for the remapping. For example, if the grid line with {\it i=1} overlaps
  ...
  this CPP key forces this rules to be respected.

\item {\tt \_\_NO\_16BYTE\_REALS}:  must be specified if you compile
with PGF90.
\end{itemize}
...
CERFACS web site at https://cerfacs.fr/en/online-traning-sessions/ .
...
\subsection{Fortran, C and python equivalent (pyoasis/examples)}
\label{subsec_equivalent}

Different examples implementing the Fortran, C and python APIs are provided as practical illustrations in directory {\tt /pyoasis/examples} :
...
\item {\tt 13-test-fracwgt} : illustrates the {\tt fracwgt} option, see appendix \ref{subsec_fracwgt} ; exchange of 3 coupling fields: the basic coupling field with its dynamic fractions as {\tt fragwgt} argument, the dynamic fraction field {\it per se}, the scaled coupling field (i.e. the basic coupling field multiplied by its dynamic fractions).
\item {\tt 14-test-inter3d} : illustrates the use of a cube partition, see section \ref{Cube_partition} .
\end{itemize}
...
If you experience problems while running your coupled model,  you can obtain more information in OASIS3-MCT debug files, by
increasing the value of the first number on the line below {\tt \$NLOGPRT} in your {\it namcouple}, see section
\ref{subsec_namcouplefirst} for details.

\subsection{Time statistics files}
\label{timestat}

The level of time statistics written out in files named {\tt *.timers\_xxxx} depends on the value of the second number on the line below the \$NLOGPRT keyword in the {\it namcouple} :

\begin{itemize}
\item 0 : timers are not activated, nothing is calculated, nothing is written.
\item 1 : timers are activated and written in a
  single file by the process 0 as well as the min and the max times
  over all the processes.
\item 2 : timers are activated and each process
  writes its own file ; process 0 also writes the min and the max
  times over all the processes in its file.
\item 3 : timers are activated and each process
...
of the meaning of the different timers implemented by default.
\footnote{Other measures can be obtained by defining the logical
...
\label{lucia}
 ...
 To facilitate this work, a load balancing analysis functionality is included in OASIS3-MCT and can be activated by setting to 1 the third number under {\tt \$NLOGPRT} in the  {\it namcouple} configuration file (see section \ref{subsec_namcouplefirst}). Some details on this functionality are provided here and more information can be found in the {\tt balancing\_documentation.pdf} file in {\tt
  ...
  When activated, the load balancing analysis functionality provides general computing information (simulation time, speed, waiting time, etc.) in a text file {\tt load\_balancing\_info.txt} for the coupled model and for each component; this global information can help to allocate resources in a balanced way. 

This functionality also outputs the full timeline of all OASIS3-MCT related events, for any of the allocated resources. This timeline is saved in one NetCDF file per coupled component, {\tt timeline\_XXX\_component.nc} where {\tt XXX} is the component name. It provides the comprehensive sequence of all operations related to the coupling (field put and get through MPI, field output on disk, field interpolation and mapping, field reading on disk, restart writing, initialisation and termination phase of the OASIS3-MCT setup) so that any simulation slow down in link with the use of the OASIS3-MCT library can be identified.
