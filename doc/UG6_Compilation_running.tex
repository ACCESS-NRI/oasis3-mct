...
With the new compiling environment available on OASIS3-MCT master branch since December 2022 (commit 5e9efd18), two environment variables {\tt \$(OASIS\_COUPLE)} and {\tt \$(OASIS\_ENV)} have to be defined. {\tt \$(OASIS\_COUPLE)} gives the location of the coupler sources ({\tt /lib, /util, /examples}, etc. directories). {\tt \$(OASIS\_ENV)} gives the extension of the header Makefile to be used for compilation {\tt make.\$(OASIS\_ENV)}. 
{\tt TopMakefileOasis3} includes {\tt make.inc}
which itself includes \newline {\tt \$(OASIS\_COUPLE)/util/make\_dir/make.\$(OASIS\_ENV)} where {\tt make.\$(OASIS\_ENV)} is specific to the hardware and compiling environment used. 
We recommend to compose {\tt \$(OASIS\_ENV)} as {\tt (computer\_name)\_(compiler\_version)\_(MPI\_library\_version)}. A file named \newline {\tt comp\_env\_\$(OASIS\_ENV).sh} must also exist the directory {\tt oasis3-mct/util/make\_dir}. It defines the computer environment for running the test examples from {\tt oasis3-mct/examples}.  

Several header files {\tt make.\$(OASIS\_ENV)} and {\tt comp\_env\_\$(OASIS\_ENV).sh} files are distributed with the release and can by used as
a template to create a custom file for your machine, see the files in directory {\tt oasis3-mct/util/make\_dir}. It is important to note that the coupled model components must be compiled with the same compilers and system software than the OASIS3-MCT library itself.
After successful compilation, resulting
libraries are found in the directory in {\tt \$(ARCHDIR)/lib}\footnote{where \$(ARCHDIR) = \$(OASIS\_COUPLE)/INSTALL\_OASIS.\$(OASIS\_ENV), see the header files} while
