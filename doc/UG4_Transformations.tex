...
When {\tt ACCUMUL} or {\tt AVERAGE} is specified for a field that is sent with a fraction weight field (see the {\tt fracwgt} option in section \ref{prismput}),
the remapping of the instantaneous field is performed, the {\tt fracwgt} term is applied as a normalization term on the target side and then the accumulation or averaging is done on the remapped fields. This may increase the cost compared to {\tt ACCUMUL}  or {\tt AVERAGE} without {\tt fracwgt}, where the remapping is done only at coupling time on the accumulated or averaged field. 

If the CPP key {\tt LOCTRANS\_RESTARTS} is activated when compiling OASIS3-MCT, partial time transformations, if any, will be saved in the coupling restart file at the end of the run for exact restart of the next run. {\bf Note that this is not backward compatible as {\tt LOCTRANS} restart fields were saved by default in OASIS3-MCT\_5.0.} Although we do not recommand it as it may have unexpected side effects, this allows a run to end in the middle of a coupling period. If used, it is important to note the following:
\begin{itemize}
...
\item In the restart file, {\tt LOCTRANS} restart fields for higher-order terms (if any, see section \ref{prismput}) are named with the symbolic name of the couplig fields prefixed with {\tt av?locxxxxxx\_} where {\tt ?} is 2, 3, 4, or 5 for the higher-order $2^{nd}$/$3^{rd}$/$4^{th}$/$5^{th}$ coupling field array respectively.
\item If the {\tt fracwgt} option is used (see section \ref{prismput}), {\tt LOCTRANS} restart fields are also saved for the destination accumulated/averaged normalised scaled field and the fraction field, named with the symbolic name of the couplig fields prefixed respectively with {\tt av1mlocxxxxxx\_} and {\tt avfwlocxxxxxx\_}.
...
\section{The remapping (also called interpolation or regridding)}
\label{subsec_interp}
...
\item {\bf CHECKOUT}:
  ...
  Note that {\tt CHECKOUT} does not provide exact weighted mean and sum when the {\tt fracwgt} option involving dynmaic fractions is activated, see section \ref{prismput}; indeed in that case, {\tt CHECKOUT} will not use the equivalent active destination fraction and a warning will be printed in the debug files.
