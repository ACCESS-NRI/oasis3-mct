...
\end{verbatim}
  ...
  LOCTRANS with min or max is applied on the raw data passed into OASIS3-MCT
  before BLASOLD is applied.  In addition,
  accumulation or averaging with fracwgt mapping requires an accumluation
  of the instanteous mapped fields.  This may increase cost somewhat
  relative to accumulation or averaging without fracwgt where the
  accumulated fields can be mapped at coupling time.  For
  averaging or accumulating, the coupler period is assumed to be constant
  and each instantaneous field is given the same weight over the
  averging/accumulating period.  In general, OASIS3-MCT does not support
  coupling with timing-varying coupling periods (timesteps).
...
\item {\bf BLASOLD}:
  ...
  occurs before the interpolation and cannot be used with min or max
  LOCTRANS.
