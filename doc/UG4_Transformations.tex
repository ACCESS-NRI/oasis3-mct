...
\item {\bf MAPPING}:
  ...
  are not needed, except if the post-processing global option
...
\end{verbatim}
 ...
 \begin{enumerate}
  ...
  \item Lambert projection above/below {\tt north\_thresh}/{\tt south\_thresh}
    ...
    {\tt \bf south\_thresh} are set to 2.0 and -2.0 respectively and the Lambert projection is not activated.} 
    (Note that {\tt north\_thresh} was set to 1.45 in previous versions prior to OASIS3-MCT\_4.0.)
    
    Of course, these values ca be changed in {\tt oasis3-mct/lib/scrip/remap\_conserv.F90} and OASIS3-MCT has then to be recompiled to consider the updated values.
