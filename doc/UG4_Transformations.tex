...
\item {\bf MAPPING}:
  ...
  In this case {\it grids.nc}, {\it masks.nc}, and {\it areas.nc}
  ...
  {\it areas.nc} and {\it masks.nc} files.
...
\item {\bf CONSERV}:
  ...
  defined in the {\it areas.nc} file.  The areas can be writtent by OASIS method {\tt oasis\_write\_area} (see section
  \ref{subsubsec_griddef}). The areas should consist of matching units
  on the source and destination side. {\bf For a correct CONSERV operation, overlapping grid cells on the source grid or on the target grid must be masked.} In the {\it namcouple}, {\tt CONSERV} requires
...
\end{verbatim}
  ...
  \item {\tt \$CMETH} is the method desired with the following choices
    ...
    \item with {\tt \$CMETH = GSSPOS}, the same operation as {\tt GLBPOS} is performed
      ...
      global conservation of the field, it does not change the sign of the
    ...
    \item with {\tt \$CMETH = BASBAL}, the operation is analogous to
      {\tt GLOBAL} as an additive term except the non masked surface of the source
