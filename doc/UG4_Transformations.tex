...
\item {\bf SCRIPR}:
 ...
 layout of the calling model but only enrols one MPI process per
...
\end{verbatim}
    ...
    \item  {\tt \$CMETH \$CGRS \$CFTYP \$REST \$NBIN \$NV} entries are
      as for {\tt DISTWGT} and  {\tt \$VAR} is as for {\tt
        GAUSWGT}. Note that for these interpolations, {\tt \$NV} represents the number of TARGET neighbours. For details, see \cite{maisonna2020}.
    \end{itemize}
...
\item {\bf CONSERV}:
 ...
 % but non conservative interpolation (i.e. bilinear) is carried out.
  ...
  This analysis requires to define the source and target grid mesh areas in file {\it areas.nc} file and the source and target grid mask in file {\it masks.nc}.
