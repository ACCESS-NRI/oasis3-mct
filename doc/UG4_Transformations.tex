...
\item {\bf CHECKIN}:
  ...
  sum of the (masked, if available) source field values.  If a grid area
  or fraction field is also available, then the area and/or fraction weighted
...
\item {\bf MAPPING}:
  ...
  Using {\tt MAPPING}, the files {\it grids.nc}, {\it masks.nc}, and {\it areas.nc}
  are not needed, except for conservative remapping
for which the normalisation by the true area of the cells is 
applied, i.e. DESTARTR, DESTNNTR, FRACARTR, or FRACNNTR (see section
\ref{subsec_interp}), in which case {\em areas.nc} file must be provided,
  and if the post-processing global option
  CONSERV is specified in the namcouple, in which case
  {\it areas.nc} and {\it masks.nc} files must be provided.
...
\item {\bf SCRIPR}:
 ...
 New in OASIS3-MCT\_4.0, a hybrid MPI+OpenMP parallel version of the SCRIP library is available. It relies on the MPI parallel layout of the calling model but only enrols one MPI process per node. The number of OpenMP threads per MPI process used by SCRIP is set by a dedicated environment variable OASIS\_OMP\_NUM\_THREADS, which can be different from the default number of threads per MPI process used elsewhere in the application, set by the environment variable OMP\_NUM\_THREADS. In practice, OASIS\_OMP\_NUM\_THREADS has to be smaller or equal to OMP\_NUM\_THREADS. For optimum performance, it is recommended to set OASIS\_OMP\_NUM\_THREADS to the number of cores on the node. The {\tt test\_interpolation} environment (see section \ref{subsec_testinterpolation}) in {\tt oasis3-mct/examples/test\_interpolation} gives a practical example on how to use the SCRIP library in parallel to calculate remapping weight-and-address files. The details of the SCRIP parallelisation can be found in \cite{piacentini08} and \cite{valcke11}\footnote{Thanks to some preliminary work, a few bugs were fixed in the SCRIP library, in particular in the bounding box definition of the grid cells. This solves an important bug observed in the Pacific near the equator for the bilinear and bicubic interpolations for Cartesian grids. However, given these modifications, one cannot expect to get exactly the same results for the interpolation weight-and-address remapping files with this new parallel SCRIP version as compared to the previous SCRIP version in OASIS3-MCT\_3.0. We checked in many different cases that the interpolation error is smaller or of the same order than before. We also observed that the parallelisation does not ensure bit reproducible results when varying the number of processes or threads.} .
  ...
  {\tt rmp\_{\it srcg}\_to\_{\it tgtg}\_{\it INTTYPE}\_{\it NORMAOPT}.nc}, 
  {\tt rmp\_{\it srcg}\_to\_{\it tgtg}\_{\it INTTYPE}\_{\it NBRS}.nc}, or 
  {\tt rmp\_{\it srcg}\_to\_{\it tgtg}\_{\it INTTYPE}.nc}.  
  {\it srcg} and {\it tgtg} are the acronyms of respetively the source and the target
  grids, {\it INTTYPE} is the interpolation type (i.e. {\tt DISTWGT}, {\tt DISTWGTNF},
  {\tt GAUSWGT}, {\tt GAUSWGTNF}, {\tt BILINEAR}, - {\bf not BILINEA as in OASIS3.3}, 
  {\tt BILINEARNF}, {\tt BICUBIC}, {\tt BICUBICNF}, or
  {\tt CONSERV}).
  Then {\it NORMAOPT} is the normalization
  option, for {\tt CONSERV} only (i.e. {\tt DESTAREA}, {\tt DESTARTR}, {\tt DESTNNEI} {\tt DESTNNTR}, {\tt FRACAREA}, {\tt FRACARTR},  {\tt FRACNNEI} or {\tt FRACNNTR}, see below). {\it NBRS} are the number of
  neighbors used for {\tt DISTWGT}, {\tt DISTWGTNF}, {\tt GAUSWGT}, or {\tt GAUSWGTNF} only.
...
\end{verbatim}
  ...
  \item {\tt BILINEAR} performs an interpolation based on a local
    ...
    interpolation for non-masked target points with a nearest neighbor fill.
...
\end{verbatim}
 ...
 \begin{enumerate}
  ...
  \item Lambert projection above/below {\tt north\_thresh}/{\tt south\_thresh}
    ...
    The analysis of the impact of the Lambert projection for specific grids can be found in \cite{valcke19}.
  ...
  \item If a target grid cell intersects only masked source cells, or falls outside the source grid domain, it
    ...
    normalisation options are used, in which case it will get the
  ...
  {\bf {\tt \bf DESTARTR}, {\tt \bf DESTNNTR}, {\tt \bf FRACARTR}, or {\tt \bf FRACNNTR} options : the ``True Area'' (TR) correction}
...
More details can be found in section 6 of \cite{valcke19}. A full validation of the TR correction can be found in that report. It is noted there that the TR correction always improves the results even if, in the cases tested, the misfit between the true area and the are evaluated by the SCRIP is always small (except for ``polar'' cells of course). Finally, we note that the fact that the TR correction cannot be applied to the ``polar” cells because of their specific treatement by the SCRIP reduces also its interest.   
...
\item {\bf CONSERV}:
 ...
 % but non conservative interpolation (i.e. bilinear) is carried out.
  ...
  defined in the {\it areas.nc} file.  The areas can be writtent with the OASIS API routine {\tt oasis\_write\_area} (see section
  \ref{subsubsec_griddef}).  {\bf The areas must be expressed in matching units
  on the source and destination side and; futhermore, these must be square radians if the TR correction is activated} (see paragraph The ``True Area'' (TR) correction in section \ref{subsec_interp}). 
  
  A grid cell mask or fractions must be defined for that operation. If both are defined, they must be consistent; OASIS3-MCT will abort if they are not or if both are missing.  Note that by OASIS conventions for the mask, a gridcell with mask=0 (active) should have fractions greater than 0 and a gridcell with mask=1 (inactive) should have fractions equal to 0.  The fraction field is written to the {\em masks.nc} file. 
...
If we follow this best practice, the atmospheric cell fractions and mask will be specific to the coupling with each particular ocean grid. As specific attribute named {\tt coherent\_with\_grid} indicating the grid prefix of the {\tt companion} grid may be defined for mask and fractions. If the OASIS API is used to define the mask and fractions, this can be done via the optional argument {\tt companion} indicating the grid prefix of the companion grid (see section \ref{subsubsec_griddef}).
...
\item {\bf CHECKOUT}:
  ...
  sum of the (masked, if available) destination field values.  If a grid area
  or fraction field is also available, then the area and/or fraction weighted
  mean and sum are also diagnosed and written.  Information about masking
  and weighting is written to the output file.  All diagnotics are
