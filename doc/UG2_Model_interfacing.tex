...
\item {\tt CALL prism\_def\_var\_proto(var\_id, name, il\_part\_id,
    var\_nodims, kinout, var\_type, ierror)} or
...
\item {\tt CALL prism\_def\_var\_proto(var\_id, name, il\_part\_id,
  ...
  \item {\tt var\_actual\_shape [INTEGER, DIMENSION(2*id\_var\_nodims(1)), IN]}:
    ...
    for backwards compatibility, it can still be passed.
...
\section{A Brief Overview of MCT}
\label{subsubsec_gridsscalars}

This section will provide a brief overview of OASIS3-MCT for coupling grids, 
mapping, and the possibility of coupling scalars.
...
MCT does not compute intepolation weights, but
...
OASIS3-MCT supports both mapping and communication of data thru MCT.  As
...
OASIS3-MCT also imposes a few other contraints on the usage.  MCT
does NOT support haloed communication.  There must be a 1-to-1 relationship
between grid point value on the source grid and on the destination grid.
...
the field decomposition cannot reference the same global gridcell ID more than once.
...
might be supported thru OASIS3-MCT but requirements and implementation are
still be assessed.

Both MCT and the OASIS layer on top of MCT have minimal OpenMP support at the
present time.  Users are generally discouraged from calling into OASIS3-MCT
from a threaded region.  And it is not possible to define a partition (decomposition)
across multiple active threads.  Most of the work that OASIS3-MCT does is MPI based
and related to decomposition rearrangement for communication or mapping.  There
...
scalar coupling, we are thinking of scalar variables such as date and time,
...
supports this type of coupling specifically, but it's possible to use the 
...
\begin{itemize}
\item Determine the number and types of scalars to couple
...
\item Broadcast the scalars after being received if needed.
\end{itemize}
