...
\item {\tt CALL prism\_get\_intracomm(new\_comm, cdnam, kinfo)}
  ...
  \item {\tt cdnam [CHARACTER*; IN]}: other component name(s) (i.e. 2nd argument of  the call to {\tt oasis\_init\_comp} in that component).  This argument can be a single character string or a 1d array of character strings (i.e. cdnam(:)).
  \item {\tt kinfo [INTEGER; OUT; OPTIONAL]}: returned error code
  \end{itemize}

  This routine sets up an MPI intra-communicator between two or more
  components, the local component and the component name(s) passed in
  {\tt cdnam} .  This call is collective
  across the tasks of the components creating the intra-communicator only.
  This method creates a new collective group of tasks with new
  ranks.  This communicator supports collective communications and is
