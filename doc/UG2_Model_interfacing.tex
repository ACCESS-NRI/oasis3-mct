...
\item {\tt CALL prism\_write\_frac (cgrid, nx\_global, ny\_global, frac, il\_part\_id), companion}
  ...
  \item {\tt companion [CHARACTER ;IN; OPTIONAL]} : the character string value associated with the frac field attribute ``coherent\_with\_grid''.  This will be written to the {\em masks.nc} netcdf file with the frac field.  It is purely informational and used in cases where the frac field is derived from or consistent with another grid.
  \end{itemize}
  Writes the component grid fraction.  This should be consistent with the mask field and will define the fraction of the grid cell that is covered by the mask.  The frac field is only used in the {\tt CONSERV} operations and either the mask or frac must be defined by the user for that operation.  If both are defined, they must be consistent.  Note that by OASIS conventions for the mask, a gridcell with mask=0 should have a frac>0 and a gridcell with mask=1 should have a frac=0.  The frac field is written to the {\em masks.nc} file.
