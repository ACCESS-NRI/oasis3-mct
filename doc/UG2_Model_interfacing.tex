...
\item {\tt CALL prism\_get\_intracomm(new\_comm, cdnam, kinfo)}
  ...
  \item {\tt kinfo [INTEGER; OUT; OPTIONAL]}: returned error code
  \end{itemize}
\item {\tt CALL oasis\_get\_intracomm(new\_comm, cdnam, kinfo, root\_ranks)}
\item {\tt CALL prism\_get\_intracomm(new\_comm, cdnam, kinfo, root\_ranks)}
  \begin{itemize}
  \item {\tt new\_comm [INTEGER; OUT]}: MPI intra-communicator
  \item {\tt cdnam [CHARACTER*; IN]}: component name(s) (i.e. 2nd argument of the call to {\tt oasis\_init\_comp} in that component).  This argument is a 1d array of character strings (i.e. cdnam(:)).
  \item {\tt kinfo [INTEGER; OUT; OPTIONAL]}: returned error code
  \item {\tt root\_ranks [INTEGER; OUT; OPTIONAL]}: this integer array must be the same size as the cdnam array.  The output has the root rank of each component in cdnam in the new communicator.
  \end{itemize}

  This routine sets up an MPI intra-communicator between two or more
  components.  In the first interface, a single other component name is passed
  in and an intra-communicator is created between the local component and the
  single other component.  In the second option, an array of component
  names is passed into the interface and an intra-communicator is created
  between them all.  In this instance, the local model name MUST BE one of the
  models defined in the {\tt cdnam} array.  The component names must 
  be valid names, but blank component names are allowed and ignored.
  This call is collective
  across the tasks of the components creating the intra-communicator only.
  This method creates a new communicator consisting of a new collective group
  of tasks with new
  ranks.  The root ranks of the components can be optionally computed
  using the {\tt root\_ranks} argument in the interface where {\tt cdnam}
  is an array.  The size of {\tt cdnam} and {\tt root\_ranks} should be
  identical.
