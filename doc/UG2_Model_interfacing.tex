...
\ref{Fig_coupling_layouts_a} ; how to use the OASIS3-MCT library
...
In {\tt examples/tutoria\_communication} and {\tt
  examples/regrid\_environment} one finds practical examples of how to use the OASIS3-MCT library
(see sections \ref{subsec_tutorial} and \ref{subsec_regrid}).

With OASIS3-MCT, it is currently possible to (in the following paragraph, the text between [ and ] refers to figure \ref{Fig_coupling_layouts_a}) :
...
and does not call any other OASIS3-MCT routine but {\tt
  oasis\_terminate}.
...
for this component tasks; in this case, {\tt oasis\_init\_comp} and {\tt oasis\_terminate} are the
only API routine that need to be called by the component tasks. If at
...
participating to the coupling will also have to call {\tt oasis\_get\_localcomm}, {\tt oasis\_create\_couplcomm}, {\tt
  oasis\_enddef} and {\tt oasis\_terminate}.
\end{itemize}
...
all processes exchanging coupling data have to describe, in a global index space spanning the 2D domain, the local
...
does not mean that 3D interpolations are supported by OASIS: this
{\tt cube} partitioning requires pre-computed interpolation weight files and
forbids any other pre or post processing transformation that supposes
the 2D dimentionality of the exchanged data.
...
The partitions for the different processes cannot overlap, i.e. haloes are not supported and it is not possible to define a partition across multiple active OpenMP threads. 
With releases earlier than OASIS3-MCT\_5.2 (2022/11/02), it was not allowed to associate more than
...
mapping matrix, OASIS3-MCT supports a partition which acts as
...
its local extents in the X and Y dimensions. The global extents in the
X and Y dimensions and the number of vertical levels must also be
...
\item {\tt ig\_paral(7)} = the number of levels.
\end{itemize}
...
The field names in the {\em grids.nc}, {\em masks.nc}, and {\em
  areas.nc} follow a well-defined convention.  The fields are normally
two-dimensional, and each field name consists of a grid acronym followed by a string that identifies the field.  For instance, the center latitudes for the grid {\it torc} will be called {\it torc.lat} and the center longitudes will be called {\it torc.lon} in the netcdf file.  The {\em grids.nc} file contains the center latitudes (.lat) and longitudes (.lon) as well as the corner latitudes (.cla) and corner longitudes (.clo).  The corner fields have a third dimension associated with the number of corners per gridcell.  The {\em area.nc} file constains the area field (.srf).  The {\em masks.nc} file contains the mask (.msk) and frac (.frc) fields.

In the the special case
of unbundled 3D fields with user provided mappings, the {\em grids.nc} and {\em
  areas.nc} files are not required, while {\em masks.nc} is only
...
\item {\tt CALL oasis\_write\_mask (cgrid, nx\_global, ny\_global, mask, il\_part\_id, companion)}
  ...
  \item {\tt companion [CHARACTER ;IN; OPTIONAL]} : the character
    string value associated with the mask field attribute {\tt coherent\_with\_grid}'.  This will be written to the {\em masks.nc} netcdf file with the mask field.  It is purely informational and used in cases where the mask field is derived from or consistent with another grid.
  \end{itemize}
  ...
  grid cell.  The mask field is used by both the {\tt SCRIPR} map
  generation function and in the global {\tt CONSERV} operations if defined.
...
\item {\tt CALL oasis\_def\_var (var\_id, name, il\_part\_id,
  ...
  \item {\tt var\_id [INTEGER; OUT]}: coupling field ID.  Note that all
      ...
      be activated and appear in the {\it namcouple}. If a field does not appear in the {\it namcouple},
  ...
  \item {\tt var\_actual\_shape [INTEGER, DIMENSION(:) or DIMENSION(:,:), IN]}:
...
\ref{subsubsec_Partition})\footnote{But the field rank 
  ...
  partition description (i.e. it can be expressed in either 1D or 2D) as long as it provides overall the same number of elements.} and the fields
can be bundled (i.e. have an extra non-spatial dimension, for example for
...
special case limited to specific
...
\begin{itemize}
\item {\tt CALL oasis\_put (var\_id,date,fld1,info,fld2,fld3,fld4,fld5,write\_restart,fracwgt)}
  \begin{itemize}
  ...
  \item {\tt write\_restart [LOGICAL, IN, OPTIONAL]}: optional argument to write an
    intermediate restart file associated with the variable {\tt
      var\_id} at the current timestep; false by default. If true, the field that exists at the time of the call is written in an intermediate restart file, taking into account LOCTRANS operations. 

If multiple fields are coupled as a single operation in the model (indicated via a list of colon-delimited fields in the namcouple, see \ref{subsubsec_secondEXPORTED}), {\tt write\_restart} should be true for the {\tt oasis\_put} calls of all corresponding fields. 

Restart files are created with 9 digits in their name, corresponding to the time of the {\tt oasis\_put} call, e.g. TA000003600 rst4.nc or TC000014400 rst4.nc. An intermediate restart file associated with a LOCTRANS operation starts with TA while an intermediate standard coupling restart file starts with TC. The coupling restart filename defined in the namcouple (e.g. rst4.nc) is used to generate the name of these intermediate restart files.

  \item {\tt fracwgt [REAL, IN, OPTIONAL]}: optional fraction weight
    field array; 4-byte or 8-byte 1D, 2D, 3D, bundle 1D, or bundle 2D; {\tt fracwgt} multiplies {\tt fld1} on the source side and then is mapped and applied as a normalization term on the destination side. It may vary in time and can represent the fraction of the cell for which {\tt fld1} is valid, like an ice fraction. Calling {\tt oasis\_put} with this optional argument is referred to as the {\tt fracwgt} option in the rest of the text.
...
\item The {\tt fracwgt} option can only be carried out with remapping on the source model processors, ({\tt \$MAPLOC=src} in section \ref{subsec_interp}) and cannot be combined with {\tt LOCTRANS T\_MIN/T\_MAX} (see section \ref{subsec_timetrans}). 
...
\item If a local time transformation is indicated for the field in the {\it namcouple} (INSTANT, AVERAGE, ACCUMUL, T\_MIN or
  ...
  performed below the {\tt oasis\_put} and the resulting field is finally sent at the coupling period.
...
This possibility must be used for example for the higher-order {\tt
  ...
  This will be used also for the higher-order remapping {\tt CONSERV/SECOND} for which a $1^{st}$, $2^{nd}$,
  $3^{rd}$
  ...
  Higher-order remappings are therefore supported given that the higher-order fields are provided at each time step as {\tt oasis\_put} arguments and given that higher-order remapping set of weights are provided in the remapping file.

  Higher-order remappings {\tt CONSERV/SECOND} or {\tt BICUBIC} can only be carried out with remapping on the source model processors, ({\tt \$MAPLOC=src} in section \ref{subsec_interp}) and cannot be combined with {\tt LOCTRANS T\_MIN/T\_MAX} (see section \ref{subsec_timetrans}). 
...
\item If a coupling field has a positive lag, the coupling field that
  matches the {\tt oasis\_get} at time=0 is automatically, in the source model under its {\tt oasis\_enddef}, read in from a coupling
  restart file and sent to match that {\tt oasis\_get}
    (see section \ref{subsec_lag}).

\item The {\tt oasis\_get} routine cannot be called from an OpenMP
threaded region.

% \item Coupling multiple fields via a single communication is supported
%  through colon delimited field lists in the namcouple (see
%  \ref{subsubsec_secondEXPORTED}).
...
\subsubsection{Coupling scalar values}
\label{subsec_scalar}
...
\item {\tt CALL oasis\_terminate (kinfo)}
  ...
  \item {\tt kinfo [INTEGER; OUT]}: returned error code.
  \end{itemize}
All processes have to terminate the coupling by
  calling this routine\footnote{If the process called {\tt MPI\_Init}
    (before calling {\tt oasis\_init\_comp}), it must also call {\tt
      ...
      oasis\_terminate}.}(normal termination). Different configurations of components and corresponding use of {\tt oasis\_terminate} are described in section \ref{sec_deploy} and on figures \ref{Fig_coupling_layouts_a} and \ref{Fig_coupling_layouts_b}.
