...
operations, see sections \ref{sec_transformations} and \ref{subsec_griddata} and 
...
\item {\tt CALL prism\_write\_mask (cgrid, nx\_global, ny\_global, mask, il\_part\_id, companion)}
  ...
  \begin{itemize}
  \item {\tt cgrid }, {\tt nx\_global }, {\tt ny\_global }, {\tt il\_part\_id }: as for {\tt oasis\_write\_grid}
  \item {\tt mask [INTEGER, DIMENSION(nx,ny) ;IN]} : mask array covering the whole grid ({\tt nx=nx\_global}, {\tt ny=ny\_global}) or only the local partition. Be careful about OASIS3-MCT historical convention (!): 0 = not masked (active), 1 = masked (not active).
...
\ref{subsubsec_Partition})\footnote{But the decomposition of a
  ...
  partition description, i.e. it can be expressed in either 1D or 2D.}  Third, the fields
can be bundled, i.e. have an extra (non-spatial) dimension (e.g. for a field covering different ice categories).  The bundle dimension is always the last dimension in the
...
\begin{itemize}
\item 1D, fld(:) = an single, unbundle field of decomposition rank 1.
...
It is possible to define a 1D bundle or 2D bundle field  with a bundle dimension of 1, i.e. for a bundle that contains only one single field.
...
If a user sets that argument to true, a ``intermediate'' restart is written for that field {\bf only for that timestep}. The {\tt write\_restart} 
