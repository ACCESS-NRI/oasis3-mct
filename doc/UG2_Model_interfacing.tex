...
mapping matrix, OASIS3-MCT provides a partition which acts as
a Box partition declaration at every vertical level without the need
of repeating it. On every
2D level, each local partition is a rectangular region of the global
...
\item {\tt CALL oasis\_put (var\_id,date,fld1,info,fld2,fld3,fld4,fld5,write\_restart,fracwgt)}
  ...
  \item {\tt info [INTEGER; OUT]}: returned info code:
    ...
    \end{itemize}
  \item {\tt fld2/fld3/fld4/fld5 [REAL, IN, OPTIONAL]}: optional $2^{nd}$/$3^{rd}$/$4^{th}$/$5^{th}$ coupling
    field array; 4-byte or 8-byte 1D, 2D, 3D, bundle 1D, or bundle 2D; rank and size
    must match {\tt fld1}; needed for high-order remapping, see details below.
...
Restart files are created with 9 digits in their name, corresponding to the time of the {\tt oasis\_put} call, e.g. TA000003600 rst4.nc or TC000014400 rst4.nc. An intermediate restart file associated with a LOCTRANS operation starts with TA while an intermediate standard coupling restart file starts with TC. The coupling restart filename defined in the namcouple (e.g. rst4.nc) is used to generate the name of these intermediate restart files.
  ...
  \item {\tt fracwgt [REAL, IN, OPTIONAL]}: optional fraction weight
    field array; 4-byte or 8-byte 1D, 2D, 3D, bundle 1D, or bundle 2D; rank and grid size must match {\tt fld1}; number of bundles must match {\tt fld1} and in that case, {\tt fracwgt} can be different for the different bundle members or be 1. 
This field multiplies {\tt fld1} on the source side and then is mapped and applied as a normalization term on the destination side. It may vary in time and can represent the fraction of the cell for which {\tt fld1} is valid, like an ice fraction.  
Calling {\tt oasis\_put} with this optional argument is referred to as the {\tt fracwgt}  option in the rest of the text.
  \end{itemize}
\end{itemize}
...
(when such a file is needed) must strictly match the number of source fields passed to the {\tt oasis\_put }.
...
This possibility must be used for example for the higher-order {\tt
  ...
  Higher-order remappings are therefore supported given that the higher-order fields are provided at each time step as {\tt oasis\_put} arguments.

\item The {\tt fracwgt} option and higher-order remappings {\tt CONSERV/SECOND} or {\tt BICUBIC} can only be carried out with remapping on the source model processors, ({\tt \$MAPLOC=src} in section \ref{subsec_interp}) and cannot be combined with {\tt LOCTRANS T\_MIN/T\_MAX} (see section \ref{subsec_timetrans}). XXXXXXXXXX

\item The {\tt fracwgt} option is implemented by multiplying {\tt fld1} by the {\tt fracwgt}, remapping that product on the target grid, then normalizing by the mapped {\tt fracwgt}. Higher-order transforms are then applied separately without regard to {\tt fracwgt}. XXXXXXXXXX 
...
\item {\tt int oasis\_c\_init\_comp(int* compid, const char* comp\_name, const bool coupled)}
  ...
  \item This {\tt oasis\_c\_init\_comp} interface includes the {\tt coupled} flag but
    NO communicator argument; use  {\tt
      oasis\_c\_init\_comp\_with\_comm} below to indicate a
