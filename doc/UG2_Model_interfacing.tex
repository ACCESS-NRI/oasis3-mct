...
components and sub-components and allows regridding (also called remapping or interpolation), time integration or accumulation and other transformations of these coupling fields.
...
The OASIS3-MCT Application Programming Interface (API) includes different classes of modules or routines that are described in detail in sections \ref{API}, \ref{APIC} and \ref{APIpython} for Fortran, C and python codes respectively.
...
sub-components can be deployed in a much larger diversity of configurations
than before. This is illustrated on figure
...
We call here an ``executable'' a compiled code
  ...
  component sending or receiving coupling fields on a specific grid. Of course, a component may have only one sub-component
  that gathers all its tasks.

In {\tt examples/tutoria\_communication}, {\tt
  examples/regrid\_environment} and \newline {\tt pyoasis/examples}, one finds practical examples of how to use the OASIS3-MCT library
(see sections \ref{subsec_tutorial} and \ref{subsec_regrid}).

With OASIS3-MCT, it is currently possible to implement coupling exchanges (in the following paragraph, the text between [ and ] refers to figure \ref{Fig_coupling_layouts_a}) :

\begin{figure}
  \includegraphics[scale=.6]{figures/coupling_layouts_a}
  \caption{The different configuration of components supported by
    OASIS3-MCT. Two executables {\tt exe1} and {\tt exe2} are running
    concurrently on separate sets of MPI tasks (0-5 for {\tt exe1} and
    6-37 for {\tt exe2}). Executable {\tt exe1} includes only one
    component {\tt comp1} that has coupling fields defined on only one
    grid {\tt grid1} (decomposed on all of its 6 tasks). Executable
    {\tt exe2} includes 3 components, {\tt comp2}, {\tt comp3}, and
    {\tt comp4} running concurrently respectively on tasks 6-11, 12-33
    and 34-37. Component {\tt comp2} participates in the coupling with
    fields defined on only one coupling grid {\tt grid2} (decomposed
    on all of its 5 tasks) while {\tt comp4} does not participate at all in the coupling.
...
not involved in the coupling. Sub-components{\tt  exe2-comp3-grid3} and
{\tt exe2-comp3-grid5}, or sub-components {\tt exe2-comp3-grid4} and
{\tt exe2-comp3-grid5} are examples of coupling between sub-components running sequentially on overlapping sets of tasks.}
...
of its component as $2^{nd}$ argument and {\tt oasis\_terminate}
before exiting. As none of {\tt comp4} tasks is
participating to the coupling, {\tt comp4} tasks calls {\tt
  oasis\_init\_comp} with {\tt coupled=.false.”} as $4^{th}$ argument
and does not call any other OASIS3-MCT routine except {\tt
  oasis\_terminate}.
As some of {\tt comp3} tasks are participating in the coupling, all {\tt comp3} tasks have to
...
\item between two components or
...
\item within one executable between
...
\item or to have some tasks of a component not participating to the coupling exchanges [tasks 31-33 of {\tt exe2-comp3}]
\item or to have all processes of a component not participating to the coupling exchanges [{\tt exe2-comp4}, tasks 34-37]
...
the Fortran codes of the components. The interface for C or C++ codes and for python codes are described in section \ref{APIC} and section \ref{APIpython} respectively.
...
Use of module {\tt
  mod\_oasis} provides access to all methods required
that will continue to evolve in the future, always
ensuring backward compatibility.  In the following sections, the
interface names are defined and
the description of the interface arguments is provided.
...
\item {\tt CALL oasis\_init\_comp (compid, comp\_name, kinfo, coupled, commworld)}
  ...
  This routine must be called by all tasks of all components whether or not they are involved in the coupling. This routine calls MPI\_Init. The component may also call MPI\_Init explicitly, but if so, has to call it before calling {\tt oasis\_init\_comp}; in this case, the component also has to call MPI\_Finalize explicitly, but only after calling {\tt oasis\_terminate}.

A component is defined as the ensemble of tasks  calling {\tt  oasis\_init\_comp} with the same {\tt comp} {\tt \_name}
argument. If, and only if, all tasks of a component are excluded from the coupling, the logical {\tt coupled} can be set to {\tt .false.}
for the tasks of this component; in this case, {\tt oasis\_init\_comp} and {\tt oasis\_terminate} are the
only API routine that need to be called by the component tasks (see component {\tt exe2-comp4} on figure \ref{Fig_coupling_layouts_b}). If at
...
component tasks have to call {\tt oasis\_init\_comp} with {\tt
  coupled=.true.} (which is the default); in this case, the component tasks not
participating to the coupling will also have to call {\tt
  oasis\_enddef} and {\tt oasis\_terminate}, and possibly {\tt oasis\_get\_localcomm} or {\tt oasis\_create\_couplcomm} if these routines are called by the other tasks of the component (see tasks 31-33 of {\tt exe2-comp3} on figure \ref{Fig_coupling_layouts_b}).
\end{itemize}
...
argument).
  ...
  This may be needed as all executables of the coupled system are usually started in a pseudo-MPMD
  mode with MPI1 and therefore share automatically the same MPI\_COMM\_WORLD
  communicator.  Another communicator has therefore to be used for the internal
...
\item {\tt CALL oasis\_create\_couplcomm(icpl, local\_comm,
  ...
  It is mandatory to call this routine that creates a coupling communicator for a subset of
  processes if only a subset of
  the component processes participate in the coupling (e.g. {\tt exe2-comp3} in figure
    \ref{Fig_coupling_layouts_b}). In that case, the processes
    involved in the coupling have to call it with icpl=1 while the
    other have to call it with icpl = {\tt MPI\_UNDEFINED}. Argument
   {\tt  local\_comm} is the MPI communicator associated with all processes
    of the component returned by {\tt oasis\_get\_localcomm}. The new coupling
    communicator is returned in {\tt coupl\_comm}.
...
\item {\tt CALL oasis\_set\_couplcomm(coupl\_comm, kinfo)}
  ...
  This routine allows users to provide a local coupling communicator to
...
all processes exchanging coupling data have to describe, in a global index space spanning the full domain, the local
partitioning of the different grids onto which the data is expressed (see \ref{subsubsec_griddef} for the grid definition).

The choices detailed in the following provide a compact description of
2D partitions. In addition, a special case was introduced in
OASIS3-MCT\_6.0 to facilitate the description of 3D partitions. This
does not mean that calculation of 3D interpolation weights are supported by OASIS3-MCT. In particular, the
{\tt cube} partition (see below) requires pre-computed 3D interpolation weights and
forbids any other pre- or post-processing transformation that supposes
a 2D structure of the exchanged data.
...
The partitions for the different processes cannot overlap, i.e. haloes are not supported and it is not possible to define a partition across multiple active OpenMP threads.\footnote{Note also that with releases earlier than OASIS3-MCT\_5.2 (2022/11/02), it was not allowed to associate more than
one grid to one partition (this could cause problems with the global {\tt CONSERV} operation.)}.
...
\item {\tt CALL oasis\_def\_partition (il\_part\_id, ig\_paral,
  ...
  \item {\tt ig\_paral [INTEGER, DIMENSION(:), IN]}: vector of
    integers describing the local grid partition in the global index space spanning the 2D domain;
    has a different expression depending on the type of the partition (Serial -no
    partition, Apple, Box, Orange, Cube and Points, see below).
  \item {\tt kinfo [INTEGER; OUT]}: returned error code.
  ...
  \item {\tt name [CHARACTER, OPTIONAL, IN]}: Optional argument associating a name to the partition,
    mandatory if {\tt oasis\_def\_partition} is called either for a grid
    decomposed not across all the processes of a component or if the related {\tt oasis\_def\_partition} are not
    called in the same order on the different component processes;
    this argument is new since OASIS3-MCT\_3.0 release and is linked to the
...
This is the choice for a grid entirely supported by only one process . In this case, we have {\tt
  ig\_paral(1:3)}:
\begin{itemize}
\item {\tt ig\_paral(1)} = 0 (indicates a Serial partition, i.e. no partition)
...
\begin{figure}
  ...
  \caption{Apple partition. We assume here that the global index starts at
...
\begin{figure}
  ...
  \caption{Box partition. We assume here that the global index starts at 0
...
\begin{figure}
  ...
  \caption{Orange partition for one process. We assume here that
...
this case, we have {\tt ig\_paral(1:N)} where {\tt N = 2 + number of
  points}.
As for Orange, 
each partition has to be defined with monotonically increasing global index values. 
% \footnote{As for the Box partition, the maximum number of segments
%   is presently 338; it can be increased by modifying the value of
%   {\tt Clim\_MaxSegments}}.
...
\subsubsection{Cube partition}
\label{Cube_partition}

For the special case of 3D fields interpolation with user-provided
remapping weights, OASIS3-MCT supports a partition which acts as
a Box partition at every vertical level without the need
of repeating it. On every
2D level, each local partition is a rectangular region of the 2D global
domain, described by the global offset of its upper left corner, and
its local extents in the X and Y 2D dimensions. The global extents in the
X and Y 2D dimensions and the number of vertical levels must also be
given. In this case, we have {\tt ig\_paral(1:7)}:
\begin{itemize} 
\item {\tt ig\_paral(1)} = 5 (indicates a Cube partition)
\item {\tt ig\_paral(2)} = the upper left corner global offset in the 2D domain
\item {\tt ig\_paral(3)} = the local extent in X
\item {\tt ig\_paral(4)} = the local extent in Y
\item {\tt ig\_paral(5)} = the global extent in X
\item {\tt ig\_paral(6)} = the global extent in Y
\item {\tt ig\_paral(7)} = the number of vertical  levels
\end{itemize}
This type of partition is adapted to specific
couplings, e.g. horizontal coarsening in the exchanges between a high-resolution
ocean model and a lower-resolution bio-geo-chemistry model.  An example of how to use this type of partition is given in  {\tt /pyoasis/examples/14-test-inter3d}. 
% {Partition definition}
\subsection{Grid data file definition}
\label{subsubsec_griddef}

Grid data files are required by OASIS3-MCT for specific
operations, see sections \ref{sec_transformations} and \ref{subsec_griddata}.
These grid data files can be created by the
user before the run or can be written directly at run time by the components with the routines described in this section.
In that last case, the corresponding routine will create a grid data files if it does not exist. If the grid data file does exist, the routine can still be used to {\bf add} grid definition in that file, but it will not {\bf overwrite} a grid definition if it already exist in the file with the same grid name.

These routines can be called only by one component process to write the whole grid or by each process holding a part of a grid. In the former case, optional argument {\tt il\_part\_id} is not needed and the arrays handling the longitudes of the grid points or corners, the latitudes of the grid points or corners, the masks, fractions and areas of the grid cells need to cover the whole grid. In the later case, the {\tt il\_part\_id} returned by {\tt oasis\_def\_partition} needs to be provided as input argument and the arrays need to cover only the local partition of the grid. 

The field names in the {\em grids.nc}, {\em masks.nc}, and {\em
  areas.nc} follow a well-defined convention (see also section \ref{subsec_griddata}).  The fields are
two-dimensional, and each field name consists of a grid acronym followed by a string that identifies the field.  For instance, the center latitudes and longitudes for the grid {\it torc} will be called respectively {\it torc.lat} and {\it torc.lon}.  The {\em grids.nc} file contains the center latitudes (.lat) and longitudes (.lon) as well as the corner latitudes (.cla) and corner longitudes (.clo).  The corner fields have a third dimension associated with the number of corners per gridcell.  The {\em area.nc} file contains the area field (.srf).  The {\em masks.nc} file contains the mask (.msk) and static fraction (.frc) fields.

Note that if all remappings are based on user-defined weights, the files {\em grids.nc} and {\em
  areas.nc}  are not required, while {\em masks.nc} is only
necessary for the {\tt CHECKIN} or {\tt CHECKOUT} diagnostics (see sections \ref{sec_transformations}).
...
\item  {\tt il\_part\_id [INTEGER, OPTIONAL; IN]}: partition ID returned by \newline {\tt oasis\_def\_partition}, see \ref{subsubsec_Partition}; needed if each component task writes its own part of the grid.
  \end{itemize}
...
\item {\tt CALL oasis\_write\_corner (cgrid, nx\_global, ny\_global, nc, clon, clat, il\_part\_id))}
  ...
  needs to be called after {\tt oasis\_write\_grid}.
...
\item {\tt CALL oasis\_write\_mask (cgrid, nx\_global, ny\_global, mask, il\_part\_id, companion)}
  ...
  \begin{itemize}
  \item {\tt cgrid }, {\tt nx\_global }, {\tt ny\_global }, {\tt il\_part\_id }: as for {\tt oasis\_write\_grid}
  \item {\tt mask [INTEGER, DIMENSION(nx,ny) ;IN]} : mask array
    covering the whole grid ({\tt nx=nx\_global}, {\tt ny=ny\_global})
  ...
  \item {\tt companion [CHARACTER ;IN; OPTIONAL]} : the character
    string value that will be associated with the mask field attribute {\tt coherent\_with\_grid}'.  This will be written to the {\em masks.nc} netcdf file with the mask field.  It is purely informational and used in cases where the mask field is derived from or consistent with another grid.
  \end{itemize}
  Writes the component grid mask.  The mask field should be consistent
  with the static fraction field (see below) and will define the 0/1 mask of the
  grid cell.  The mask field is used by both the {\tt SCRIPR} remappings and in the global {\tt CONSERV} operations if defined.
  The mask field is written to the {\em masks.nc} file.
...
\item {\tt CALL oasis\_write\_frac (cgrid, nx\_global, ny\_global, frac, il\_part\_id, companion)}
  ...
  \begin{itemize}
  \item {\tt cgrid }, {\tt nx\_global }, {\tt ny\_global }, {\tt il\_part\_id }: as for {\tt oasis\_write\_grid}
  \item {\tt frac [REAL, DIMENSION(nx,ny) ;IN]} : single or double real fraction array covering the whole grid ({\tt nx=nx\_global}, {\tt ny=ny\_global}) or only the local partition.
  \item {\tt companion [CHARACTER ;IN; OPTIONAL]} : the character
    string value that will be associated with the fraction field attribute
    ``coherent\_with\_grid''.  It should refer to the acronym of
    the grid which mask was used to define the fraction of the current
    grid (see section \ref{subsec_cooking}). This will be written to the {\em
      masks.nc} NetCDF file with the fraction field.  It is purely
    informational and used in cases where the fraction field is derived
    from or consistent with another grid mask.

  \end{itemize}
  Writes the component grid cell static fractions.  This should be consistent
  with the mask field and defines the fraction of the grid cell that
  is active (i.e. not masked), e.g. the water fraction of an atmospheric cell having water and land fractions.  The fraction field is only used in the
  global {\tt CONSERV} operations. Either the mask or fractions must be defined for that operation.  If both are defined, they must be consistent; OASIS3-MCT will abort if they are not coherent or if both are missing.  Note that by OASIS3-MCT conventions for the mask, a gridcell with mask=0 (active) should have a fractions greater than 0 and a gridcell with mask=1 (inactive) should have a fractions equal to 0.  The fraction field is written to the {\em masks.nc} file.
...
\item {\tt CALL oasis\_write\_area (cgrid, nx\_global, ny\_global, area, il\_part\_id)}
  ...
  The surfaces may be given in any units but they must be coherent on
...
coupling do not have to call this routine at all but for backward
...
\item {\tt CALL oasis\_def\_var (var\_id, name, il\_part\_id,
  ...
  \item {\tt var\_id [INTEGER; OUT]}: coupling field ID.  Note that all
      coupling fields appearing in the {\it namcouple} must be declared with
      ...
      be activated in the {\it namcouple}. If a field does not appear in the {\it namcouple},
      the {\tt var\_id} returned by the {\tt oasis\_def\_var} will be -1.
      The value of the {\tt var\_id} should be tested and if {\tt var\_id}
      equals -1, the corresponding
      {\tt oasis\_put} and {\tt oasis\_get} should not be called. These constraints are imposed to avoid that a typo in the {\it namcouple}
        leads to coupling exchanges not corresponding to what the user intends to activate.
  ...
  \item {\tt var\_nodims [INTEGER, DIMENSION(2); IN]}: is an integer array of size two.
    ...
    fields or greater than 1 for fields that are bundled;
    if var\_nodims(2)=0, it will be automatically reset to 1 in the
    routine, to ensure backward compatibility).
  \item {\tt kinout [INTEGER; IN]}: {\tt OASIS\_In} 
    (i.e. = 21) for fields received by the component; {\tt OASIS\_Out}
    (i.e. = 20) for fields sent by the component
    \footnote{Parameters OASIS\_In, OASIS\_Out
      are defined in
      oasis3-mct/lib/psmile/src/mod\_oasis\_parameters.F90}.
  \item {\tt var\_actual\_shape [INTEGER, DIMENSION(:) or DIMENSION(:,:), IN]}: optional, not used anymore but can still be passed for
    backwards compatibility; the interface is overloaded to support a
    1d or 2d array of any length.
  \item {\tt var\_type [INTEGER; IN]}: type of coupling field array;
    put {\tt OASIS\_Real} (i.e. = 4) for single
    ...
    single precision data is supported by the API.
  \item {\tt kinfo [INTEGER; OUT]}: returned error code.
  \end{itemize}
\end{itemize}
...
OASIS-MCT API.  This coupling interface supports several ranks and
types of coupling fields. 
The field decomposition must be
consistent with the grid partition (see
\ref{subsubsec_Partition})\footnote{But the field rank 
  does not necesseraly have to match the rank of the grid
  partition description (i.e. it can be expressed in either 1D or 2D) as long as it provides overall the same number of elements.} and the bundle fields
are supported (i.e. have an extra non-spatial dimension, for example for
different ice categories).

The bundle field option can be used to
send or receive together multi-level variables, multiple related fields, and other types
of fields.  The fields must share a common partition and common namcouple settings (e.g. interpolation)
to be bundled. While this is a useful feature for multi-level fields, this does not mean
...
the bundle field variables are internally renamed and a field number is appended
...
It is possible to define a 1D bundle or 2D bundle field  with a bundle dimension of 1.
So in general, {\tt fld}, the field passed into the get and put interface can have rank
1, 2, or 3 and includes the following possible options:

\begin{itemize}
\item 1D, fld(:) = a single, unbundle field of rank 1.
\item 2D, fld(:,:) = a single, unbundle field of rank 2.
\item 3D, fld(:,:,:) = a single, unbundled field of decomposition rank 3 ; 
special case linked with the Cube partition (see section \ref{Cube_partition}) adapted to specific
couplings (e.g. horizontal coarsening in the exchanges between a high-resolution
ocean model and a lower-resolution bio-geo-chemistry model)
that requires user-defined interpolation remapping files, 
i.e. {\tt MAPPING} ({\em cf.} section \ref{subsec_interp}) but no vertical interpolation.
\item 1D bundle, fld(:,:) = a bundle set of fields of rank 1
...
\item 2D bundle, fld(:,:,:) = a bundle set of fields of rank 2.
  (the size of the third dimension must equal the number of fields in the
  bundle, defined by var\_nodims(2) in the {\tt oasis\_def\_var} interface).
\end{itemize}

%With bundle fields, it is important that the number of fields in the bundle match
%in the two models being coupled.  This requires that the var\_nodims(2) values
%in the send and receive model match the bundle dimension of that bundle coupling field.
For a given bundle, the
number of fields must match in the source and target models.  This is explicitly
checked within the coupling layer and will lead to an abort if the match is not effective. 

\subsubsection{Sending a coupling field (or writing a coupling restart file)}
\label{prismput}
...
\begin{itemize}
\item {\tt CALL oasis\_put (var\_id,date,fld1,info,fld2,fld3,fld4,fld5,write\_restart, \newline fracwgt)}
  \begin{itemize}
  ...
  \item {\tt date [INTEGER; IN]}: number of seconds (or any other time
    units as long as they are the same in all components and in the {\it
      namcouple}) at the time of the call 
  \item {\tt fld1 [REAL, IN]}: coupling (or I/O) field array; can be
    4-byte or 8-byte 1D, 2D, 3D, bundle 1D, or bundle 2D, see above.
  \item {\tt info [INTEGER; OUT]}: returned info code:
    \begin{itemize}
    \item {\tt OASIS\_Sent} (=4) : the field was sent to another component
    \item {\tt OASIS\_LocTrans} (=5) : the field was only used in a time
      transformation (not sent, not output)
    \item {\tt OASIS\_ToRest} (=6) : the field was written to a restart
      file only
    \item {\tt OASIS\_Output} (=7) : the field was written to an output
      file only
    \item {\tt OASIS\_SentOut} (=8) : the field was both written to an
      output file and sent to another component
    \item {\tt OASIS\_ToRestOut} (=9): the field was written both to a
      restart file and to an output file.
    \item {\tt OASIS\_WaitGroup} (=14) : the field was not sent because it is part of a group (see "Support to group multiple fields in a single communication" in section \ref{subsubsec_secondEXPORTED}.
    ...
    \end{itemize}
  \item {\tt fld2/fld3/fld4/fld5 [REAL, IN, OPTIONAL]}: optional higher-order $2^{nd}$/$3^{rd}$/$4^{th}$/$5^{th}$ coupling
    field array; 4-byte or 8-byte 1D, 2D, 3D, bundle 1D, or bundle 2D; rank and size
    must match {\tt fld1}; needed for higher-order remapping, see details below.
  \item {\tt write\_restart [LOGICAL, IN, OPTIONAL]}: optional argument to write the field array associated with {\tt var\_id} in an intermediate restart file  at the current timestep, taking into account LOCTRANS operations; false by default.
If multiple fields are grouped in a single operation in the model (see "Support to group multiple fields in a single communication" in section \ref{subsubsec_secondEXPORTED}), {\tt write\_restart} should be true for the {\tt oasis\_put} calls of all corresponding fields. Intermediate restart files are created with 9 digits in their name, corresponding to the time of the {\tt oasis\_put} call, e.g. TC000014400\_rst4.nc, where "rst4.nc" is the coupling restart filename defined in the {\it namcouple}.

  \item {\tt fracwgt [REAL, IN, OPTIONAL]}: optional fraction weight
    field array; 4-byte or 8-byte 1D, 2D, 3D, bundle 1D, or bundle 2D; it may vary in time and represents the fraction of the cell for which {\tt fld1} is valid, like an ice fraction. Calling {\tt oasis\_put} with this optional argument is referred to as the {\tt fracwgt} option in the rest of the text and is described in details in Appendix \ref{subsec_fracwgt}.

\begin{itemize}

\item {\tt fracwgt} rank must match {\tt fld1} rank; {\tt fracwgt} grid size must match {\tt fld1} grid size; if {\tt fld1} has a bundle dimension, then {\tt fracwgt} must have a bundle dimension that can be the same than {\tt fld1} bundle dimension (and in that case, {\tt fracwgt} can be different for the different bundle members) or that can be equal to 1 (and in that case, {\tt fracwgt} is the same for the different bundle members). 

\item {\tt fracwgt} cannot be combined with global CONSERV (see section \ref{subsec_cooking}) if  {\tt LOCTRANS AVERAGE} or {\tt ACCUMUL} is also activated, or with {\tt LOCTRANS T\_MIN} or {\tt T\_MAX} (see section \ref{subsec_timetrans}). 

\item If the {\tt fracwgt}  option is activated for a field with a LAG (see section \ref{subsec_namcouplesecond}), special care have to be taken on the content of the coupling restart files; see section \ref{subsec_restartdata} for details.

\item {\tt fracwgt} option can only be carried out with remapping on the source model processors, ({\tt \$MAPLOC=src} in section \ref{subsec_interp}).

\item If higher-order transforms ({\tt CONSERV/SECOND} or {\tt BICUBIC}) are specified for the coupling fields, they are applied on the target grid without regard to {\tt fracwgt}.
...
\item This routine may be called by the component at each timestep. The
  ...
  {\em namcouple}, if any, with {\tt LAG=0} by default) to the {\tt date} corresponds to a 
  a coupling period, as specified by
  the user in the {\it namcouple} (see section \ref{sec_namcouple}). By convention, the first coupling
  of a run occurs at {\tt date=0}.
...
\item For a coupling field with a positive lag, a coupling restart file containing the initial values of the field must be provided and will be used at the beginning of the run (see section
  \ref{subsec_restartdata}) is automatically overwritten by the {\tt
    oasis\_put} when the {\tt date+LAG=runtime}. {\bf It is the user's responsibility to ensure that the overwritten restart file is copied in the working directory of the next run.}
    
\item The total run time should match an integer number of coupling periods. No warranty is given when a run terminates in the middle of a coupling period, although compiling with the CPP key {\tt LOCTRANS\_RESTARTS} ensures that partial time transformations, if any, are saved in the coupling restart file at the end of the run.

\item If a local time transformation is indicated for the field in the {\it namcouple} (INSTANT, AVERAGE, ACCUMUL, T\_MIN or
  T\_MAX, see section \ref{sec_transformations}), it is automatically
  performed below the {\tt oasis\_put} and the resulting field is sent at the coupling period.
  
  %  For non-instantaneous transformations, partially
  % transformed fields will be written to the restart file at the end of
 % the run for use on the next component startup, when needed.  For
  % average or accumulating, the coupler period is assumed to be constant
  % and each instantaneous field is given the same weight over the
  % averging/accumulating period.

\item A $2^{nd}$, $3^{rd}$, $4^{th}$ and $5^{th}$ source field can be
  passed as optional arguments for higher-order remapping. If {\tt fld3}  is passed, {\tt fld2}  must also be passed; if {\tt fld4}  is passed, {\tt fld3}  and {\tt fld2}  must also be passed; if {\tt fld5}  is passed {\tt fld4}, {\tt fld3}  and {\tt fld2} must also be passed. If passed, the $2^{nd}$, $3^{rd}$, $4^{th}$ and $5^{th}$ set of weights present in the remapping file will be applied to these fields respectively (see section \ref{subsec_mapdata} for the remapping file format). 
  From OASIS3-MCT\_4.0 onwards, the number of weights in the remapping file and the number of fields in the coupling restart file
(when such a file is needed) must strictly match the number of source fields passed to the {\tt oasis\_put }. The higher-order restart fields in the coupling restart file must have the same name than the coupling field itself prefixed with either {\tt av2\_} , {\tt av3\_} , {\tt av4\_} , or {\tt av5\_} , see also section \ref{subsec_restartdata}.

This possibility of passing higher-order fields must be used for example for the higher-order {\tt
  ...
  This will be used also for the higher-order remapping {\tt SCRIPR CONSERV/SECOND} for which a $1^{st}$, $2^{nd}$,
  $3^{rd}$
  set of weights must be respectively applied to the field value, its
...
\item A coupling field sent by a source component can be
  associated with more than one target field and component, with different entries in the {\it namcouple} configuration
  ...
  same field. If coupling restart files are required (either if a {\tt
    LAG} is specified for that field), it is
  mandatory to specify different files for the different fields.

\item The {\tt oasis\_put} routine cannot be called from OpenMP-threaded regions.
...
\item {\tt CALL oasis\_get (var\_id, date, fld, info)}
  ...
  \item {\tt fld [REAL, OUT]}: I/O or coupling field array;
    can be 1D, 2D, 3D, bundle 1D, or bundle 2D.
  \item {\tt info [INTEGER; OUT]}: returned info code:
    \begin{itemize}
...
\item If a coupling field has a positive lag, the coupling field that
  matches the {\tt oasis\_get} at time=0 is automatically read in from a coupling
  restart file in the source model under its {\tt oasis\_enddef} and sent to match that {\tt oasis\_get}
    (see section \ref{subsec_lag}).

\item The {\tt oasis\_get} routine cannot be called from an OpenMP
threaded region.
...
\item {\tt CALL oasis\_terminate (kinfo)}
  ...
  \item {\tt kinfo [INTEGER; OUT]}: returned error code.
  \end{itemize}
All processes have to terminate the coupling by calling this routine. Note that if the process called {\tt MPI\_Init}, necessarily before calling {\tt oasis\_init\_comp}), it must also call {\tt MPI\_Finalize} explicitly, but only after calling {\tt oasis\_terminate}. Different configurations of components and corresponding use of {\tt oasis\_terminate} are described in section \ref{sec_deploy} and on figures \ref{Fig_coupling_layouts_a} and \ref{Fig_coupling_layouts_b}.
...
\item {\tt CALL oasis\_abort (compid, routine\_name, abort\_message, file, line, rcode)}
  ...
  \item {\tt rcode [INTEGER, OPTIONAL; IN]}: Optional argument. When OASIS3-MCT
...
\item {\tt CALL oasis\_get\_intracomm(new\_comm, cdnam, kinfo)}
  ...
  \item {\tt cdnam [CHARACTER*; IN]}: single string argument containing the other component name (i.e. 2nd argument of the call to {\tt oasis\_init\_comp} in that component). 
  \item {\tt kinfo [INTEGER; OUT; OPTIONAL]}: returned error code
  \end{itemize}
...
\item {\tt CALL oasis\_get\_multi\_intracomm(new\_comm, cdnam, root\_ranks, kinfo)}
  \begin{itemize}
  \item {\tt new\_comm [INTEGER; OUT]}: MPI intra-communicator
  \item {\tt cdnam [CHARACTER*; IN]}: 1d array of character strings (i.e. cdnam(:)) containing the name of the component (i.e. 2nd argument of the call to {\tt oasis\_init\_comp} in each component). 
  \item {\tt root\_ranks [INTEGER; OUT]}: 1d integer array (i.e. root\_ranks(:)) of the same size than {\tt cdnam} containing root ranks.  This argument is a 
  \item {\tt kinfo [INTEGER; OUT]}: returned error code
  \end{itemize}
  ...
  {\tt oasis\_enddef}.
  This method creates a new communicator consisting of a new collective group
  of tasks with new ranks.  The root rank of each individual components relative
...
inquire what would happen to the field corresponding to that
{\tt var\_id} if it was sent with an {\tt oasis\_put} at that same {\tt date}. This maybe useful if, for example, the calculation of
...
This routine returns the number of coupling exchanges in which the
field with that {\tt var\_id} is
...
\item {\tt CALL oasis\_get\_freqs(var\_id, mop, ncpl, cpl\_freqs, kinfo)}
  ...
  \item {\tt var\_id [INTEGER; IN]}: field ID (from
  corresponding oasis\_def\_var)
  \item {\tt mop [INTEGER; IN]}: OASIS\_Out or OASIS\_In
  ...
  of field var\_id. There is one coupling period for
...
the return code is consistent with the {\tt
  kinfo} argument in the Fortran interfaces (see section \ref{API}). However, for the put and get communication functions, the return error code only indicates
...
\item {\tt int oasis\_c\_init\_comp(int* compid, const char* comp\_name, const bool coupled)}
  \begin{itemize}
  \item This {\tt oasis\_c\_init\_comp} interface includes the {\tt coupled} flag but no {\tt commworld}
    argument; use  {\tt
      oasis\_c\_init\_comp\_with\_comm} below to provide a
...
\item {\tt int oasis\_c\_create\_couplcomm(const int icpl, const MPI\_Comm local\_comm, MPI\_Comm* coupl\_comm)}
  \begin{itemize}
  \item {\tt icpl} is a C {\tt int} type input argument (see {\tt
      oasis\_create\_couplcomm} in section \ref{subsubsec_Initialisation})
...
\item {\tt int oasis\_c\_put(const int var\_id, const int date, const
  ...
  \item {\tt x\_size} and {\tt y\_size} are the dimensions of the local
...
\vspace{0.2cm}
\item {\tt int oasis\_c\_put\_with\_frac(const int var\_id, const int date, const int x\_size, const int y\_size, const int bundle\_size, const int fkind, const int storage, const void* fld1, const void* fracwgt, const bool write\_restart, int* kinfo)}
  \begin{itemize}
  \item Same as for {\tt oasis\_c\_put} with the additional {\tt fracwgt} argument, which gives a fraction weight field array (see {\tt oasis\_put} in section \ref{subsubsec_sendingreceiving}). 
  \end{itemize}
\vspace{0.2cm}
...
The source code of pyOASIS is in the directory  {\tt pyoasis/src}. Complete
documentation is available in  {\tt pyoasis/pyoasis.pdf}. The pyoasis interface
ultimately call the Fortran version.  This is done by wrapping the
Fortran in ISO-C bindings (see  {\tt lib/cbindings/fortran\_isoc}), then
wrapping the Fortran ISO-C bindings in C (see  {\tt lib/cbindings/c}), then
wrapping the C bindings in python (see  {\tt pyoasis/src}).  This method
provides both the C and python bindings.  Examples on how
to use pyOASIS are provided in  {\tt pyoasis/examples}. To run all tests including python, C, and
Fortran examples, use {\tt make test}. The python wrapper functions
...
If the global communicator at the start of the run is different from
the default  {\tt MPI\_COMM\_WORLD} communicator, the global
communicator has to be passed to the {\tt Component} class through the
third optional argument. By default, the
...
see section \ref{subsubsec_Partition} and examples {\tt 1-serial},
{\tt 2-apple}, {\tt 3-box}, {\tt 4-orange}, {\tt 5-points} in {\tt pyoasis/examples}.
...
specified (see example {\tt 1-serial})
...
will be stored locally (see example  {\tt 2-apple}).
...
rectangle (see example in {\tt 3-box}) .

The Orange partitioning consists of several segments of a linear domain (see an example with only one segment per process in  {\tt 4-orange}.)

The last type of partitioning is Points, where we have to
specify, in a list, the global indices of the points stored by the
process (see example in {\tt 5-points}).
...
about the grid definition can be found in section \ref{subsubsec_griddef}. A full example of writing a grid in sequential and
parallel models can be found in {\tt examples/10-grid} .
...
different possible return codes are listed in section \ref{subsubsec_auxroutines}.
...
namcouple. See section  \ref{prismput} for details.
...
Or, if the {\tt write\_restart} argument is passed, 

\begin{verbatim}
   ____________________________________________________________
   variable.put(date, field, write_restart)
   ____________________________________________________________
\end{verbatim}

If the {\tt fracwgt} option is activated, the dynamic fraction field also has to be provided as a {\tt pyoasis.asarray} object and sent together with the field with the   {\tt put} function.
...
Finally, on the target side, the field is received with the {\tt get} function, which fills also a {\tt pyoasis.asarray} object.
...
for the cases where a voluntary abort is needed in the code when
an exception has been raised. Its interface mimics the
corresponding OASIS3\_MCT function {\tt oasis\_abort}.
...
\begin{figure}[h]
  \includegraphics[scale=.54]{figures/corresp_terminate.pdf}
...
\section{Additional notes on coupling functionality}
\label{sec_notes}
...
available interfaces.  The most robust implementation is to setup
...
\item Define coupling field names for use in both the model components and the namcouple file and declare variables as usual.  Create a set of namcouple inputs without any mapping.
...
one cannot send the scalar data to multiple processes as this violates the halo
restriction in MCT.  
%Users need to be aware which processes contain valid scalar
...
to reproduce different coupling algorithms defining {\tt LAG} values for the different coupling fields . See section \ref{subsec_namcouplesecond} for more details on how to specify a {\tt LAG} for a coupling field.
...
expressed in the time unit used and can be
...
{\tt date} argument is that same coupling time.
...
the coupling.  See section \ref{subsec_restartdata} for details on the structure of the coupling restart files. For a field with positive lag, the source
...
On figure \ref{fig:lag_concept_1}, short black arrows correspond to
  ...
  A coupling algorithm exploiting the LAG concept, is
  ...
  In conclusion, using appropriate LAG indices ensures that 
 the target component receives the coupling fields produced by the
  source component at a previous time. This is, in some coupling
...
Receiving data is always blocking while sending data is non-blocking with respect to the component making that call.  It
is possible to deadlock the system if the relative orders of puts and
gets in different components are not compatible.

The sequence (SEQ) index in the {\it namcouple} file
...
models, each one sending a field to the other at every coupling period
...
In this case, there is no sequencing dependency and specifying SEQ indices is not needed\footnote{In that case, if set, the SEQ indices
must be identical.  If by mistake, SEQ is set to 1 for fld1 and 2 for fld2,
then the coupled model will abort because at runtime, the coupler will
detect in model 2 that fld2 was sent before fld1 was received which is out of sequence as defined by the SEQ settings.}.
...
OASIS3-MCT leverages the MCT coupling library
developed at Argonne National Laboratory.  That library is designed
to couple fields on static grids between model components.  The fields
can be decomposed across mutiple processes, and the decomposition
and number of proceses involved can be arbitrary in each component.  

MCT also provides the ability to map (i.e. interpolate or
regrid) data between grids as long as the interpolation is linear and can
be computed by a linear sparse matrix multiply.
MCT does not compute interpolation weights, but
...
A coupling field can be interpolated to
...
component (see {\tt \$MAPLOC}  in section \ref{subsec_interp}). At the present time, it is not possible to map the field as part
of the communication rearrangement, although that capability should
be possible to implement.
The separation of mapping and communication is handled by the OASIS
layer.  Users only need to be aware of a few options that can be set to fine
tune the performance of these operations.

OASIS3-MCT also imposes a few other constraints on the usage.  For example, MCT
does not support haloed communication.  The partitions defined by the component that describes
the field decomposition cannot reference the same global grid cell more than once.

Also, neither OASIS3-MCT nor MCT support dynamically varying grids nor dynamically varying
decompositions at the present time.  

Finally, MCT itself, and therefore the communication layer in OASIS, has no OpenMP support. Users are discouraged from calling OASIS3-MCT {\tt oasis\_put} and {\tt oasis\_get}
from a threaded region.  And it is not possible to define a partition (decomposition) across multiple active threads. 
 The work that OASIS3-MCT does related to decomposition rearrangement for communication or mapping per se is MPI-based.  There
are some floating point operations in the sparse matrix multiple as well as in
some of the OASIS3-MCT transformation operations; but generally, these are highly parallel and
would often not benefit from use of OpenMP.  The only use of OpenMP in  OASIS3-MCT is within the mixed MPI+OpenMP parallel version of the SCRIP library for the calculation of the remapping weights (see section \ref{subsec_interp}). 
