...
With OASIS3-MCT, it is currently possible to (the text between [ and ] refers to figure \ref{Fig_coupling_layouts_a}) :
...
Note that with releases earlier than OASIS3-MCT\_5.2 (2022/11/02), it was not allowed to associate more than
...
types of coupling fields.  First, the fields passed to the interface can be
4 byte or 8 byte reals. The field decomposition must be
consistent with the decomposition defined by the grid partition (see
\ref{subsubsec_Partition})\footnote{But the decomposition of a
  field does not necesseraly have to match the rank of the grid
  partition description (i.e. it can be expressed in either 1D or 2D).} and the fields
can be bundled (i.e. have an extra non-spatial dimension for something
like different ice categories).  The bundle dimension is always the last dimension in the
field passed to the get and put routines.  And the size of the bundle dimension
...
\begin{itemize}
\item 1D, fld(:) = a single, unbundle field of decomposition rank 1.
\item 2D, fld(:,:) = a single, unbundle field of decomposition rank 2.
\item 1D bundle, fld(:,:) = a bundle set of fields of decomposition rank 1.
...
\item 2D bundle, fld(:,:,:) = a bundle set of fields of decomposition rank 2.
...
Different bundle fields
can have different numbers of fields, but for a given bundle field, the
...
It is possible to define a 1D bundle or 2D bundle field  with a bundle dimension of 1, for a bundle that contains only one single field.

Finally, the bundle field option can be used to
...
to be bundle.  While this is a useful feature for multi-level fields, {\bf this does not mean
...
\item {\tt CALL prism\_put\_proto(var\_id, date, fld1, info, fld2,
    fld3, fld4, fld5, write\_restart), fracwgt}
  \begin{itemize}
  ...
  \item {\tt fld1 [REAL, IN]}: coupling (or I/O) field array; can be
    1D, 2D, bundle 1D, or bundle 2D, see above.
  ...
  \item {\tt fld2 [REAL, IN, OPTIONAL]}: optional $2^{nd}$ coupling
    field array; can be 1D, 2D, bundle 1D, or bundle 2D.  Rank and size
    must match {\tt fld1}.
  ...
  \item {\tt fracwgt [REAL, IN, OPTIONAL]}: optional fraction weight
    field array; can be 1D, 2D, bundle 1D, or bundle 2D.  Can vary with different
    fields in a bundle or be identical for different fields in a bundle.  Grid size 
    must match {\tt fld1} and number of bundled fields must match {\tt fld1} or be 1.
    This field multiplies {\tt fld1} on the source side and then is mapped
    and applied as a normalization term on the destination side.  It will
    often vary in time and may be associated with a field like ice fraction.
  \end{itemize}
\end{itemize}
...
\item A $2^{nd}$, $3^{rd}$, $4^{th}$ and $5^{th}$ source field can be
  ...
  in the second dimension, and its cross-gradient in that order. For  {\tt
    SCRIPR/BICUBIC},  {\tt fld2}, {\tt fld3} and {\tt fld4} are
  therefore mandatory.

  This will be used also for the {\tt CONSERV/SECOND} for which a 1st, 2nd, 3rd
  ...
  in that order. For  {\tt CONSERV/SECOND},  {\tt fld2} and {\tt fld3} are
  therefore mandatory.
...
\item   BLASOLD is not allowed with higher order transforms because it is not uniquely
  defined.  Application of additive or multiplicative transforms might be better done in 
  the components or with BLASNEW with higher order transforms.

\item   Higher order remapping weights and fracwgt can only be carried out with 
  mapping on the src model processors, ({\tt \$MAPLOC=src}) in section \ref{subsec_interp}.

\item   Higher order remapping weights and fracwgt cannot be used with the min/max the
  loctrans option.  see section \ref{subsec_timetrans}.

\item Fracwgt is implemented by multiplying the fld1 by the fracwgt, mapping that product,
  then normalizing by the mapped fracwgt weight on the destination grid.  Higher order
  transforms are then applied separately without regard to fracwgt.

\item Note that from OASIS3-MCT\_4.0 onwards,
...
\item If a local time transformation is indicated for the field by the
  ...
  I/O frequency.  For non-instantaneous transformations, partially
...
\item Coupling multiple fields via a single communication is supported through colon delimited field lists in the namcouple (see \ref{subsubsec_secondEXPORTED}).
...
for all fields. This is useful to reduce communication costs by aggregating multiple fields into a single communication when multiple fields have the same coupling transformations.
...
If a user sets that argument to true, an ``intermediate'' restart is written for that field {\bf only for that timestep}. The {\tt write\_restart}
saves the data that exists at the time of the call, taking into account LOCTRANS operations. In
...
Restarts are created with a 9 digit timestamp in
their filename, corresponding to the time in seconds of the oasis\_put
call, e.g. TA000003600\_rst4.nc or TC000014400\_rst4.nc.
A restart file that starts with TA is a restart file associated
with LOCTRANS operations; a restart
file that starts with TC is a restart file associated with
coupling operations. The coupling restart filename
defined in the namcouple (e.g. rst4.nc) is used to generate the
filename of these intermediate restart files.
\end{itemize}
...
\item If a coupling field has a positive lag, the coupling field that
  ...
  restart file and sent to match that  {\tt oasis\_get} under the {\tt
    oasis\_enddef} of the source model (see section \ref{subsec_lag}).
...
As described elsewhere, OASIS3-MCT leverages the MCT 2.11 coupling infrastructure
