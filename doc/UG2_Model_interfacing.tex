...
\item   If BLASOLD is used with
  higher order transforms or fractions, BLASOLD is only applied only to fld1 before mapping.
  Use of BLASOLD transforms is not well defined with higher order remapping weights, it may
  be clearer to apply the transform on the receive side via BLASNEW.  

\item   Higher order remapping weights and fracwgt can only be carried out with 
  mapping on the src model processors, see section \ref{subsec_interp}.
