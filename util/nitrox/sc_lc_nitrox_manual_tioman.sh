...
export nitrox_oasis_root=${SCRATCHDIR}/oasis3-mct/util/nitrox
...
for nitrox_example in ${StringArrayv1[@]}; do
   ...
   sed -i 's+OASIS_ROOT=${SCRATCHDIR}+OASIS_ROOT=${NEW_OASIS_ROOT}+g' env_tests_param_${nitrox_env}
   sed -i 's+OASIS_COUPLE=${OASIS_ROOT}/oasis3-mct+OASIS_COUPLE=${HOME}/${nitrox_oasis_root}+g' env_tests_param_${nitrox_env}
 "PATHS changed in env_tests_param_${nitrox_env}"
