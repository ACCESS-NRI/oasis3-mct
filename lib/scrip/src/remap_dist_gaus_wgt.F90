...
!$OMP PRIVATE(rl_local_variance,ib_nb,rl_nb_distance,il_nb_pairs) &
#ifdef REMAP_TIMING
...
!$                dl_tstart = OMP_GET_WTIME()
            ...
            if ( ll_gaussian_weights  ) THEN
               rl_local_variance = 0.
               il_nb_pairs = 0
               do n = 1, num_neighbors-1
                  do ib_nb = 2, num_neighbors
                     if (grid1_mask(nbr_add(n)) .and. grid1_mask(nbr_add(ib_nb))) then
                        rl_nb_distance = sinlat(ib_nb)*sinlat(n) +   &
                                         coslat(ib_nb)*coslat(n)*    &
                                         (coslon(ib_nb)*coslon(n) +  &
                                         sinlon(ib_nb)*sinlon(n))
            ...
            do n=1,num_neighbors
               if (grid1_mask(nbr_add(n)) .or. lextrapdone) then
                 ...
                 if ( ll_gaussian_weights .and. il_nb_pairs > 0 ) &
                   nbr_dist(n) = exp(.5*nbr_dist(n)*nbr_dist(n)/rl_local_variance)
