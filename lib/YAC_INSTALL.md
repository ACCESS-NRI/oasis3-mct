...
YAC can be downloaded from the [DKRZ gitlab repository](https://gitlab.dkrz.de/dkrz-sw/yac). At least version 3.1.0 is needed
Version 3.1.0 has been mirrored on the [CERFACS redmine](https://inle.cerfacs.fr/projects/oasis3-mct/files)
...
Issue the following command
```
../configure --enable-lib-core-only --with-yaxt-root=${YAXT_ROOT} --with-netcdf-root=${NETCDF4_DIR} --prefix=${YAC_ROOT}
```

If the use of the YAC weight generation has to be triggered by PyOasis, dynamic libraries are needed.
In such a case add the following option to the configure line:
```
--with-pic
```
