...
module cbindings
  ...
  contains
    ...
    function stringarray_to_fortran(n, string_c)
      ...
      allocate(character(len=maxlen) :: stringarray_to_fortran(n))
      stringarray_to_fortran(:) = ' '
      do ib_str = 1, n
         call c_f_pointer(string_c(ib_str), p_char, [MAX_LENGTH])
         strlen = 0
         do i = 1, MAX_LENGTH
            if (p_char(i)/=C_NULL_CHAR) then
               strlen = strlen + 1
               stringarray_to_fortran(ib_str)(strlen:strlen) = p_char(i)
            end if
            if (p_char(i)==C_NULL_CHAR) exit
         end do
      end do

    end function stringarray_to_fortran
