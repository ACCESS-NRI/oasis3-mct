...
int oasis_c_put(const int var_id, const int kstep, const int x_size, const int y_size, const int z_or_bundle_size, const int fkind, const int storage, const void* fld1, const bool write_restart, int* kinfo){
  ...
  if ( storage == OASIS_COL_MAJOR || storage_is1D ) {
    if ( fkind == OASIS_Real ) {
      oasis_put_iso_float(&var_id, &kstep, &x_size, &y_size, &z_or_bundle_size, (float*)fld1, kinfo, &write_restart);
    } else if ( fkind == OASIS_Double ) {
      oasis_put_iso_double(&var_id, &kstep, &x_size, &y_size, &z_or_bundle_size, fld1, kinfo, &write_restart);
    } else {
	...
	for (j=0; j<y_size; j++) {
      ...
      oasis_put_iso_float(&var_id, &kstep, &x_size, &y_size, &z_or_bundle_size, fld2, kinfo, &write_restart);
      free(fld2);
    } else if ( fkind == OASIS_Double ) {
      double* fld2;
      int i,j,k;
	...
	for (j=0; j<y_size; j++) {
      ...
      oasis_put_iso_double(&var_id, &kstep, &x_size, &y_size, &z_or_bundle_size, fld2, kinfo, &write_restart);
      free(fld2);
    } else {
      oasis_c_abort(0, "oasis_c_put", "fkind argument can only be OASIS_Real or OASIS_Double", __FILE__, __LINE__);
    }
...
int oasis_c_get(const int var_id, const int kstep, const int x_size, const int y_size, const int z_or_bundle_size, const int fkind, const int storage, void* fld1, int* kinfo){
  ...
  if ( storage == OASIS_COL_MAJOR || storage_is1D ) {
    if ( fkind == OASIS_Real ) {
      oasis_get_iso_float(&var_id, &kstep, &x_size, &y_size, &z_or_bundle_size, (float*)fld1, kinfo);
    } else if ( fkind == OASIS_Double ) {
      oasis_get_iso_double(&var_id, &kstep, &x_size, &y_size, &z_or_bundle_size, fld1, kinfo);
    } else {
    ...
    if ( fkind == OASIS_Real ) {
      ...
      int i,j,k;
      fld2 = (float*) malloc(x_size*y_size*z_or_bundle_size*sizeof(float));
      oasis_get_iso_float(&var_id, &kstep, &x_size, &y_size, &z_or_bundle_size, fld2, kinfo);
      for (i=0; i<x_size; i++) {
	for (j=0; j<y_size; j++) {
	  for (k=0; k<z_or_bundle_size; k++) {
	    memcpy(fld1+(i*y_size*z_or_bundle_size+j*z_or_bundle_size+k)*sizeof(float), fld2+k*y_size*x_size+j*x_size+i, sizeof(float));
	  }
	}
      }
      free(fld2);
      ...
      int i,j,k;
      fld2 = (double*) malloc(x_size*y_size*z_or_bundle_size*sizeof(double));
      oasis_get_iso_double(&var_id, &kstep, &x_size, &y_size, &z_or_bundle_size, fld2, kinfo);
      for (i=0; i<x_size; i++) {
	for (j=0; j<y_size; j++) {
	  for (k=0; k<z_or_bundle_size; k++) {
	    memcpy(fld1+(i*y_size*z_or_bundle_size+j*z_or_bundle_size+k)*sizeof(double), fld2+k*y_size*x_size+j*x_size+i, sizeof(double));
	  }
	}
      }
      free(fld2);
