...
MODULE mod_oasis_yac_map
   ...
   ! Counters for interpolation checks
   INTEGER, SAVE :: count_pos
   INTEGER, TARGET, ALLOCATABLE :: tgt_counts(:)
...
CONTAINS
   ...
   SUBROUTINE oasis_map_yac_genmap(mapID,namID)
      ...
      ! NetCDF extra attributes
      INTEGER :: ncid
      CHARACTER(LEN=8) :: nc_date
      CHARACTER(LEN=80) :: nc_att
      CHARACTER(LEN=1024) :: nc_str
      ...
      IF (OASIS_debug >= 12) WRITE(nulprt,'(2A,I3)') TRIM(subname), &
      ...
      IF (comm_rank == 0) THEN
         ...
         nc_att = 'src_grid_repr'
         ierror = nf90_put_att(ncid, NF90_GLOBAL, &
            & TRIM(nc_att), TRIM(namyacmet(namID)%src_grid_type))
         nc_att = 'dst_grid_repr'
         ierror = nf90_put_att(ncid, NF90_GLOBAL, &
            & TRIM(nc_att), TRIM(namyacmet(namID)%dst_grid_type))
         DO ib_s = 1, namyacmet(namID)%stacksize
            WRITE(nc_att, '(A,I2.2)') 'map_method_',ib_s
   ...
   !> Callback for the YAC interpolation check method.
   SUBROUTINE update_tgt_count(global_ids, coordinates_xyz, tgt_count, user_data) BIND(c)

      USE, INTRINSIC :: iso_c_binding

      TYPE(c_ptr), VALUE            :: global_ids
      TYPE(c_ptr), VALUE            :: coordinates_xyz
      INTEGER(kind=c_size_t), VALUE :: tgt_count
      TYPE(c_ptr), VALUE            :: user_data

      tgt_counts(count_pos) = INT(tgt_count)
      count_pos = count_pos + 1

   END SUBROUTINE update_tgt_count
