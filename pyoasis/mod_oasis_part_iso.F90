...
subroutine oasis_def_partition_iso(id_part, kparal, kinfo) bind(C)
  use iso_c_binding, only: c_int, c_ptr
  use pyoasis
  use mod_oasis
  implicit none
  integer (c_int), intent(out) :: id_part
  integer (c_int), dimension(3), intent(in) :: kparal
...
!  type(c_ptr), intent(in) :: name
 ...
 ! integer :: ig_size_f
  ...
  do i=1, 3!size(kparal)
    kparal_f(i)=kparal(i)
  end do
!  ig_size_f=ig_size
!  name_f=string_to_fortran(name)

call oasis_def_partition(id_part_f, kparal_f, kinfo_f)
  id_part=id_part_f
