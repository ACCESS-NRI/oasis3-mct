...
void put(int var_id, int kstep, int size1, int size2, int size3, int kind, void* field, int *kinfo, const bool write_restart){
  if ( kind == 4) {
    oasis_put_iso_float(&var_id, &kstep, &size1, &size2, &size3, (float*)field, kinfo, &write_restart);
  } else {
    oasis_put_iso_double(&var_id, &kstep, &size1, &size2, &size3, field, kinfo, &write_restart);
  }
}
