...
class Grid(object):
    ...
    def __init__(self, cgrid, nx_global, ny_global, lon, lat, partition=None):
        """Constructor"""
        pyoasis.checktypes.check_types([str, int, int, numpy.ndarray, numpy.ndarray ,pyoasis.Partition],
                                       [cgrid, nx_global, ny_global, lon, lat, partition])
        if len(cgrid) == 0:
        ...
        Grid._counter += 1
        pyoasis.mod_oasis_grid.write_grid(self._cgrid, self._nx_glo, self._ny_glo, 
                                          lon, lat, self._part_id)
