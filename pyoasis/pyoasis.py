...
class Component:
    def __init__(self, i_name, coupled=True, i_communicator=MPI.COMM_WORLD):
        ...
        rv = mod_oasis_method_core.init_comp(self.name, coupled,
                                             self.communicator)
        error = rv[1]
        if(error < 0):
...
class OrangePartition(Partition):
    def __init__(self, offsets, extents):
        n = len(offsets)
        if(len(extents) != n):
            raise OasisException("Number of offsets != number of extents")
        parameters1 = [3, n]
        for i in range(n):
...
class Var:
    def __init__(self, i_id_part, cdport, i_id_var_nodims1,
        ...
        kinout = i_kinout
        ktype = i_ktype
        rv = mod_oasis_var_core.def_var(id_part, self.name, id_var_nodims1,
