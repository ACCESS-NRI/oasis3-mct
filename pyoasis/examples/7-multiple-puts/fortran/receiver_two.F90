program receiver_two
  ...
  integer :: var_id(2), var_nodims(2), var_actual_shape(1), date
  ...
  if(kinfo<0) &
     & call oasis_abort(comp_id, comp_name, "Error in oasis_init_comp: ", rcode=kinfo)

  part_params=(/0, 0, n_points/)
  call oasis_def_partition(part_id, part_params, kinfo)
  if(kinfo<0) call oasis_abort(comp_id, comp_name, "Error in oasis_def_partition: ", rcode=kinfo)
	
  var_nodims=(/1, 1/)
  var_actual_shape(:)=1

  do  i = 1,2
     print *, "Recv_two: defining var: ", var_name(i)
     ...
     if(kinfo<0 .or. var_id(i)<0) &
        & call oasis_abort(comp_id, comp_name, "Error in oasis_def_var: "//trim(var_name(i)), rcode=kinfo)
  end do
  
  call oasis_enddef(kinfo)
  if(kinfo<0) call oasis_abort(comp_id, comp_name, "Error in oasis_enddef: ", rcode=kinfo)

  call oasis_get_ncpl(var_id(1), ncpl, kinfo)
  if(kinfo<0) call oasis_abort(comp_id, comp_name, "Error in oasis_get_ncpl: ", rcode=kinfo)

  allocate(cpl_freqs_1(ncpl))
  call oasis_get_freqs(var_id(1), OASIS_IN, ncpl, cpl_freqs_1, kinfo)
  if(kinfo<0) call oasis_abort(comp_id, comp_name, "Error in oasis_get_freqs: ", rcode=kinfo)
  ...
  do date = 0, 43200
     ...
     if (any(mod(date,cpl_freqs_1) == 0)) then
        ...
        if(kinfo<0) &
           & call oasis_abort(comp_id, comp_name, "Error in oasis_get: "//trim(var_name(1)), rcode=kinfo)

        error = sum(abs(field(:)-date))
        if(error<epsilon) then
           print '(A,I0)', "Recv_two: field 1 received successfully at time ",date 
        else
     ...
     if (any(mod(date,cpl_freqs_2) == 0)) then
        ...
        if(kinfo<0) &
           & call oasis_abort(comp_id, comp_name, "Error in oasis_get: "//trim(var_name(2)), rcode=kinfo)

        error = sum(abs(field(:)+date))
        if(error<epsilon) then
           print '(A,I0)', "Recv_two: field 2 received successfully at time ",date 
        else
           print '(A,I0,A,F6.0,A,F6.0)', &
  ...
  if(kinfo<0) call oasis_abort(comp_id, comp_name, "Error in oasis_terminate: ", rcode=kinfo)

end program receiver_two
